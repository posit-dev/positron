...
function renderFooter(report) {
	const parts = report.cost.passes.map(p => {
		const bits = [
			p.cost,
			p.turns && (p.maxTurns ? `${p.turns}/${p.maxTurns} turns` : `${p.turns} turns`),
			p.duration,
		].filter(Boolean).join(' &middot; ');
		return `<span>${p.label} ${bits}</span>`;
	});
	if (report.cost.total) {
		parts.push(`<span>total ${report.cost.total}</span>`);
	}
	return parts.length ? `<footer class="cost">${parts.join('')}</footer>` : '';
}
