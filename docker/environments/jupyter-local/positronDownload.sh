...
# Optional: set TAG to force a specific release (e.g., TAG=2026.05.0-95)
TAG="${TAG:-}"
...
# --- Pick the positron-server tarball for our arch ---
# Expected pattern: positron-server-linux-<arch>-<tag>.tar.gz
asset_json="$(printf '%s' "$rel_json" | jq -r --arg arch "$ARCH" '
  .assets[]
  | select(.name
      | test("^positron-server-linux-" + $arch + "-[0-9.\\-]+\\.tar\\.gz$"))
')"

if [[ -z "$asset_json" ]]; then
  echo "No matching positron-server tarball found for arch '$ARCH' in release $TAG"
