...
- **Unmapped dirs:** if your PR touches a Positron dir with no entry in the map, the E2E Tests comment will note it so you (or a maintainer) can add a mapping. When the dir is upstream and only holds a few Posit-owned files, map the dir to `[]` and add a tagged entry per file -- the longest matching key wins, so upstream changes in that dir stay untagged.
