...
- **Source / extension changes** map to feature tags via [`.github/workflows/e2e-tag-paths-map.json`](https://github.com/posit-dev/positron/blob/main/.github/workflows/e2e-tag-paths-map.json) (for example, a change under `contrib/positronConsole/` adds `@:console`).
- **Changed e2e test files** contribute the feature tags they already declare (their `tags.XXX`), so editing a test reruns the suite it belongs to.
