...
exports.cloneTestRepo = cloneTestRepo;
exports.copyFixtureFile = copyFixtureFile;
const cp = __importStar(require("child_process"));
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
const rimraf_1 = __importDefault(require("rimraf"));
const os = __importStar(require("os"));
function cloneTestRepo(workspacePath = process.env.WORKSPACE_PATH || 'WORKSPACE_PATH is not set in cloneTestRepo') {
    ...
    if (process.env.FORCE_CLONE === 'true') {
        if (!remoteCommitHash) {
            ...
            rimraf_1.default.sync(cacheDir);
        }
    }
...
function copyRepo(source, destination) {
 ...
 */
async function copyFixtureFile(fixtureFilename, destinationFolder, replaceCtrl = false) {
