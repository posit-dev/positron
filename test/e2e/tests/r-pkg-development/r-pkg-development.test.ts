...
test.describe('R Package Development', { tag: [tags.WEB, tags.R_PKG_DEVELOPMENT] }, () => {
	...
	test('R - Verify can open, test, check, install, and restart package', async function ({ app, openFolder, logger }) {
		...
		await test.step('Test R Package', async () => {
			...
			await expect(async () => {
				await app.workbench.terminal.waitForTerminalText('[ FAIL 1 | WARN 0 | SKIP 0 | PASS 16 ]', { web: app.web });
				await app.workbench.terminal.waitForTerminalText('Terminal will be reused by tasks', { web: app.web });
		...
		await test.step('Check R Package', async () => {
			...
			await expect(async () => {
				await app.workbench.terminal.waitForTerminalText('Error: R CMD check found ERRORs', { web: app.web });
				await app.workbench.terminal.waitForTerminalText('Terminal will be reused by tasks', { web: app.web });
			}).toPass({ timeout: 70000 });
		});
