...
_test_setup_1.test.describe('R Debugging', {
    tag: [_test_setup_1.tags.DEBUG, _test_setup_1.tags.WEB, _test_setup_1.tags.WIN, _test_setup_1.tags.ARK]
}, () => {
    _test_setup_1.test.beforeAll('Setup fruit data', async ({ executeCode, sessions }) => {
    ...
    _test_setup_1.test.afterEach('Reset for next test', async ({ hotKeys, app }) => {
        await hotKeys.closeAllEditors();
        await app.workbench.console.clearButton.click();
    });
    (0, _test_setup_1.test)('R - Verify call stack behavior and order', async ({ app, page }) => {
    ...
    (0, _test_setup_1.test)('R - Verify debugger indicator/highlight maintains focus during code execution', {
    ...
    (0, _test_setup_1.test)('R - Verify debugging with `browser()` via console', async ({ app, page, openFile, runCommand, executeCode }) => {
        const { debug, console } = app.workbench;
        await openFile(`workspaces/r-debugging/fruit_avg_browser.r`);
        await runCommand('r.sourceCurrentFile');
        // Trigger the breakpoint
    ...
    (0, _test_setup_1.test)('R - Verify debugging with `browser()` via debugging UI tools', async ({ app, page, openFile, runCommand, executeCode }) => {
        const { debug, console } = app.workbench;
        await openFile(`workspaces/r-debugging/fruit_avg_browser.r`);
        await runCommand('r.sourceCurrentFile');
        // Trigger the breakpoint
    ...
    (0, _test_setup_1.test)('R - Verify debugging with `debugonce()` pauses only once', async ({ app, page, executeCode, openFile, runCommand }) => {
        const { debug, console } = app.workbench;
...
_test_setup_1.test.describe('R Breakpoints', {
    tag: [_test_setup_1.tags.DEBUG, _test_setup_1.tags.WIN, _test_setup_1.tags.ARK]
}, () => {
    let breakpointSession;
    _test_setup_1.test.beforeAll(async ({ sessions }) => {
        breakpointSession = await sessions.start('r');
    });
    _test_setup_1.test.afterEach(async ({ app, page, hotKeys, cleanup }) => {
        // Focus the console
    ...
    (0, _test_setup_1.test)('R - Verify breakpoint set and hit via gutter click', {
        tag: [_test_setup_1.tags.WEB]
    ...
    (0, _test_setup_1.test)('R - Verify breakpoints in dirty (unsaved) documents', async ({ app, page, openFile, hotKeys }) => {
        const { debug, console } = app.workbench;
        await openFile('workspaces/r-debugging/breakpoint_test.r');
        await debug.setUnverifiedBreakpointOnLine(3);
    ...
    (0, _test_setup_1.test)('R - Verify session switching preserves breakpoint state', {
        tag: [_test_setup_1.tags.WEB]
    ...
    (0, _test_setup_1.test)('R - Verify debug-specific console history', {
        tag: [_test_setup_1.tags.WEB],
    ...
    (0, _test_setup_1.test)('R - Verify DAP disconnect/reconnect preserves breakpoints', {
        tag: [_test_setup_1.tags.WEB]
    }, async ({ app, page, openFile, hotKeys }) => {
        const { debug, console } = app.workbench;
        await openFile('workspaces/r-debugging/breakpoint_test.r');
    ...
    (0, _test_setup_1.test)('R - Verify editing file while at breakpoint invalidates breakpoints', async ({ app, page, openFile, hotKeys }) => {
        const { debug, console } = app.workbench;
        await openFile('workspaces/r-debugging/breakpoint_test.r');
        await debug.setUnverifiedBreakpointOnLine(3);
        // Verify breakpoint
...
async function verifyDebugPane(app) {
...
async function verifyCallStack(app) {
    const { debug } = app.workbench;
    await debug.expectCallStackAtIndex(0, 'fruit_avg(');
    await debug.expectCallStackAtIndex(1, '<global>');
}
async function verifyVariableInConsole(app, name, expectedText) {
    await _test_setup_1.test.step(`Verify variable in console: ${name}`, async () => {
        await app.workbench.console.focus();
        await app.code.driver.currentPage.keyboard.type(name);
        await app.code.driver.currentPage.keyboard.press('Enter');
        await (0, _test_setup_1.expect)(app.code.driver.currentPage.getByText(expectedText)).toBeVisible({ timeout: 30000 });
    });
}
//# sourceMappingURL=r-debug.test.js.map