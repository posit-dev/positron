...
test.describe('Cell Deletion Action Bar Behavior', {
	...
	test('Cell deletion using action bar', async function ({ app, settings }) {
		...
		await app.positron.notebooksPositron.addCodeToCellAtIndex('# Cell 5', 5);
		...
		await app.positron.notebooksPositron.deleteCellWithActionBar(2);

		// Verify cell count decreased
		expect(await getCellCount(app)).toBe(5);

		// Verify what was cell 3 is now at index 2
		expect(await app.positron.notebooksPositron.getCellContent(2)).toBe('# Cell 3');
		...
		expect(await app.positron.notebooksPositron.getCellContent(3)).toBe('# Cell 4');

		// Delete the selected cell using action bar
		await app.positron.notebooksPositron.deleteCellWithActionBar(3);
		...
		expect(await app.positron.notebooksPositron.getCellContent(2)).toBe('# Cell 3');
		expect(await app.positron.notebooksPositron.getCellContent(3)).toBe('# Cell 5');
		...
		await app.positron.notebooksPositron.deleteCellWithActionBar(0);

		// Verify cell count decreased
		expect(await getCellCount(app)).toBe(2);

		// Verify what was cell 1 is now at index 0
		expect(await app.positron.notebooksPositron.getCellContent(0)).toBe('# Cell 1');
		expect(await app.positron.notebooksPositron.getCellContent(1)).toBe('# Cell 3');
		...
		expect(await app.positron.notebooksPositron.getCellContent(0)).toBe('# Cell 3');
