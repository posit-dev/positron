...
_test_setup_js_1.test.describe('Positron Notebooks: Kernel Behavior', {
    tag: [_test_setup_1.tags.WIN, _test_setup_1.tags.WEB, _test_setup_1.tags.POSITRON_NOTEBOOKS, _test_setup_1.tags.SOFT_FAIL] // soft fail due to https://github.com/posit-dev/positron/issues/10546
}, () => {
    (0, _test_setup_js_1.test)('ensure notebook session states update correctly during start, restart, and shutdown', async function ({ app }) {
    ...
    (0, _test_setup_js_1.test)('ensure variable and output persistence after kernel restart', async function ({ app }) {
    ...
    (0, _test_setup_js_1.test)('ensure new notebooks use foreground session kernel', async function ({ app, sessions }) {
        const { notebooksPositron } = app.workbench;
    ...
    _test_setup_js_1.test.skip('ensure existing notebooks use their correct interpreter kernel', {
        annotation: [{ type: 'issue', description: 'https://github.com/posit-dev/positron/issues/7593' }] // <--- test is failing on chromium only
    }, async function ({ app, sessions }) {
        const { notebooksPositron } = app.workbench;
        const pythonNotebook = path_1.default.join('workspaces', 'data-explorer-update-datasets', 'pandas-update-dataframe.ipynb');
        const rRnotebook = path_1.default.join('workspaces', 'r_notebooks', 'Introduction+to+R.ipynb');
        // start multiple sessions and select R
        const [, rSession] = await sessions.start(['python', 'r']);
        await sessions.select(rSession.id);
    ...
    (0, _test_setup_js_1.test)('ensure notebook console attaches and terminates with active kernel', async function ({ app, sessions, settings }) {
...
//# sourceMappingURL=notebook-kernel-behavior.test.js.map