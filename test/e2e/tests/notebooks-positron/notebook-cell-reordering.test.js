...
_test_setup_js_1.test.describe('Notebook Cell Reordering', {
    ...
    // --- Drag-and-Drop Tests ---
    (0, _test_setup_js_1.test)('Drag handle: visible on hover, hidden otherwise', async function ({ app }) {
        const { notebooksPositron } = app.workbench;
        // Setup: Create notebook with 3 cells
        await notebooksPositron.newNotebook({ codeCells: 3 });
        await notebooksPositron.expectCellCountToBe(3);
    ...
    _test_setup_js_1.test.skip('Drag-and-drop: swap 1st and 2nd cell', async function ({ app }) {
        const { notebooksPositron } = app.workbench;
        // Setup: Create notebook with 3 cells
        await notebooksPositron.newNotebook({ codeCells: 3 });
        await notebooksPositron.expectCellContentsToBe(['# Cell 0', '# Cell 1', '# Cell 2']);
    ...
    (0, _test_setup_js_1.test)('Drag-and-drop: reorder markdown cells', async function ({ app }) {
        const { notebooksPositron } = app.workbench;
    ...
    (0, _test_setup_js_1.test)('Drag-and-drop: move cell to end', async function ({ app }) {
        const { notebooksPositron } = app.workbench;
        // Setup: Create notebook with 4 cells
        await notebooksPositron.newNotebook({ codeCells: 4 });
        await notebooksPositron.expectCellContentsToBe(['# Cell 0', '# Cell 1', '# Cell 2', '# Cell 3']);
    ...
    (0, _test_setup_js_1.test)('Drag-and-drop: move cell from end to beginning', async function ({ app }) {
        const { notebooksPositron } = app.workbench;
        // Setup: Create notebook with 4 cells
        await notebooksPositron.newNotebook({ codeCells: 4 });
        await notebooksPositron.expectCellContentsToBe(['# Cell 0', '# Cell 1', '# Cell 2', '# Cell 3']);
    ...
    (0, _test_setup_js_1.test)('Drag-and-drop: undo restores original order', async function ({ app }) {
        const { notebooksPositron } = app.workbench;
        // Setup: Create notebook with 3 cells
        await notebooksPositron.newNotebook({ codeCells: 3 });
        await notebooksPositron.expectCellContentsToBe(['# Cell 0', '# Cell 1', '# Cell 2']);
    ...
    (0, _test_setup_js_1.test)('Drag-and-drop: redo reapplies reorder', async function ({ app }) {
        const { notebooksPositron } = app.workbench;
        // Setup: Create notebook with 3 cells
        await notebooksPositron.newNotebook({ codeCells: 3 });
        await notebooksPositron.expectCellContentsToBe(['# Cell 0', '# Cell 1', '# Cell 2']);
    ...
    (0, _test_setup_js_1.test)('Drag-and-drop: escape cancels drag operation', async function ({ app }) {
        const { notebooksPositron } = app.workbench;
        const keyboard = app.code.driver.currentPage.keyboard;
        await notebooksPositron.newNotebook({ codeCells: 3 });
        await notebooksPositron.expectCellContentsToBe(['# Cell 0', '# Cell 1', '# Cell 2']);
    ...
    (0, _test_setup_js_1.test)('Drag-and-drop: auto-scroll when dragging in long notebook', async function ({ app }) {
        const { notebooksPositron } = app.workbench;
    ...
    (0, _test_setup_js_1.test)('Multi-drag: drag multiple selected cells together', async function ({ app }) {
        const { notebooksPositron } = app.workbench;
        const keyboard = app.code.driver.currentPage.keyboard;
        // Create notebook with 5 cells
    ...
    _test_setup_js_1.test.skip('Multi-drag: single cell drag ignores multi-selection when dragging unselected cell', async function ({ app }) {
        const { notebooksPositron } = app.workbench;
        const keyboard = app.code.driver.currentPage.keyboard;
        // Create notebook with 5 cells
        await notebooksPositron.newNotebook({ codeCells: 5 });
    ...
    (0, _test_setup_js_1.test)('Multi-drag: undo/redo multi-cell drag operation', async function ({ app }) {
        const { notebooksPositron } = app.workbench;
        const keyboard = app.code.driver.currentPage.keyboard;
    ...
    (0, _test_setup_js_1.test)('Multi-drag: drag three cells to beginning of notebook', async function ({ app }) {
        const { notebooksPositron } = app.workbench;
        const keyboard = app.code.driver.currentPage.keyboard;
