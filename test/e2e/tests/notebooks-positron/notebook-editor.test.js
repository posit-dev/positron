...
const NOTEBOOK_PATH = path_1.default.join('workspaces', 'bitmap-notebook', 'bitmap-notebook.ipynb');
_test_setup_js_1.test.use({
    suiteId: __filename
});
_test_setup_js_1.test.describe('Positron Notebooks: Open & Save', {
    tag: [_test_setup_1.tags.WIN, _test_setup_1.tags.POSITRON_NOTEBOOKS]
}, () => {
    (0, _test_setup_js_1.test)('Switching between VS Code and Positron notebook editors works correctly', async function ({ app, hotKeys, settings }) {
    ...
    (0, _test_setup_js_1.test)('Positron notebooks can open new untitled notebooks and saving works properly', { tag: [_test_setup_1.tags.WEB] }, async function ({ app, runCommand, cleanup }) {
        ...
        await quickInput.type(path_1.default.join(app.workspacePathOrFolder, baseFileName));
    ...
    (0, _test_setup_js_1.test)('Ghost editor issue: Positron notebook does not create duplicate VS Code notebook on reload with dirty notebook', async function ({ app, hotKeys }) {
        ...
        await (0, test_1.expect)(tabsBefore).toHaveCount(1);
        ...
        await (0, test_1.expect)(tabsAfter).toHaveCount(1);
        ...
        await (0, test_1.expect)(positronNotebookElements).toHaveCount(1);
        await (0, test_1.expect)(vscodeNotebookElements).toHaveCount(0);
