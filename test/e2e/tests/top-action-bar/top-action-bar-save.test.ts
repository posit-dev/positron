...
test.describe('Top Action Bar - Save Actions', {
	...
	test('Save and Save All both disabled when no unsaved editors are open', async function ({ app }) {
		...
		await expect(async () => {
			...
			expect(await app.workbench.topActionBar.saveAllButton.isDisabled()).toBeTruthy();
	...
	test('Save enabled and Save All disabled when a single unsaved file is open', async function ({ app }) {
		...
		await expect(async () => {
			expect(await app.workbench.topActionBar.saveButton.isEnabled()).toBeTruthy();
			expect(await app.workbench.topActionBar.saveAllButton.isEnabled()).toBeTruthy();
