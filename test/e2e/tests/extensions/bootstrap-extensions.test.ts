...
async function waitForExtensions(extensions: { fullName: string; shortName: string; version: string }[], extensionsPath: string) {
	...
	if (mismatched.size > 0) {
		...
		console.log(' 1. Download outdated package from Open VSX Registry: https://open-vsx.org/extension/posit/publisher');
		console.log(' 2. If SHA is needed, follow instructions here: https://connect.posit.it/positron-wiki/updating-extensions.html#updating-extensions');
		console.log(' 3. Update the `product.json` file with the correct version and SHA');
