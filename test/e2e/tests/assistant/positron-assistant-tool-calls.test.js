 ...
 */
_test_setup_1.test.describe('Positron Assistant Tool Scoping', { tag: [_test_setup_1.tags.WIN, _test_setup_1.tags.ASSISTANT, _test_setup_1.tags.WEB] }, () => {
    ...
    _test_setup_1.test.beforeAll('Sign in to Assistant', async function ({ app }) {
        await app.workbench.assistant.openPositronAssistantChat();
        await app.workbench.assistant.loginModelProvider('echo');
    });
    _test_setup_1.test.beforeEach('Clean up sessions', async function ({ app }) {
     ...
     */
    (0, _test_setup_1.test)('R-specific tools are excluded when only Python session is active', async function ({ app, sessions }) {
        // Start only a Python session (reuse: false ensures fresh session)
        await sessions.start('python', { reuse: false });
        await _test_setup_1.test.step('Send message and verify R tools are not available', async () => {
            ...
            for (const tool of R_SPECIFIC_TOOLS) {
                (0, _test_setup_1.expect)(availableTools, `R-specific tool '${tool}' should not be available when only Python session is active`).not.toContain(tool);
            }
            // Verify that Python-specific tools ARE available
            for (const tool of PYTHON_SPECIFIC_TOOLS) {
                (0, _test_setup_1.expect)(availableTools, `Python-specific tool '${tool}' should be available when Python session is active`).toContain(tool);
            }
        });
    });
    /**
     ...
     */
    (0, _test_setup_1.test)('Python-specific tools are excluded when only R session is active', async function ({ app, sessions }) {
        // Start only an R session (reuse: false ensures fresh session)
        await sessions.start('r', { reuse: false });
        await _test_setup_1.test.step('Send message and verify Python tools are not available', async () => {
            await app.workbench.assistant.clickNewChatButton();
            await app.workbench.assistant.selectChatMode('Agent');
            // Send a simple message to get the available tools
            await app.workbench.assistant.sendChatMessageAndWait('Hello');
            ...
            for (const tool of PYTHON_SPECIFIC_TOOLS) {
                (0, _test_setup_1.expect)(availableTools, `Python-specific tool '${tool}' should not be available when only R session is active`).not.toContain(tool);
            }
            // Verify that R-specific tools ARE available
            for (const tool of R_SPECIFIC_TOOLS) {
                (0, _test_setup_1.expect)(availableTools, `R-specific tool '${tool}' should be available when R session is active`).toContain(tool);
            }
        });
    });
    /**
     ...
     */
    (0, _test_setup_1.test)('Non-session-specific tools are available when any session is active', async function ({ app, sessions }) {
        ...
        await sessions.start('python', { reuse: false });
        await _test_setup_1.test.step('Verify non-session-specific tools are available', async () => {
            await app.workbench.assistant.clickNewChatButton();
            await app.workbench.assistant.selectChatMode('Agent');
            await app.workbench.assistant.sendChatMessageAndWait('Hello');
            const availableTools = await app.workbench.assistant.getAvailableTools(app.workspacePathOrFolder);
            ...
            for (const tool of GENERIC_SESSION_TOOLS) {
                (0, _test_setup_1.expect)(availableTools, `Generic session tool '${tool}' should be available when any session is active`).toContain(tool);
            }
            // Verify that non-session tools ARE available
            for (const tool of NON_SESSION_TOOLS) {
                (0, _test_setup_1.expect)(availableTools, `Non-session tool '${tool}' should be available`).toContain(tool);
            }
        });
    });
});
//# sourceMappingURL=positron-assistant-tool-calls.test.js.map