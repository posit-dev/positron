...
test.describe('Quarto - Inline Output: Notebook Statement Range (Python)', {
	...
	// doesn't interfere with other tests that rely on the python fixture.
	test('Python - Multi-line statement execution works within Quarto document', async function ({ app, openFile, sessions, hotKeys }) {
		...
		await test.step('Open Quarto document and run first cell to start notebook session', async () => {
			...
			// Run the first cell to start the notebook Python session
		...
		await test.step('Execute multi-line statement with Ctrl+Enter', async () => {
			...
			// language delegates to 'python' providers by creating a vdoc.
			// If no provider matches the vdoc, only line 19 is sent and
			// Python returns a SyntaxError for the incomplete expression.
			...
			// print(1 + 2 + 3) = "6". If only line 19 was sent,
			// we'd see a SyntaxError instead.
			await inlineQuarto.expectOutputContainsText('6', { index: 1, timeout: 30000 });
		});
	});
});
