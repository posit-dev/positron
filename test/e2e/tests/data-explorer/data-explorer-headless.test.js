...
_test_setup_1.test.describe('Headless Data Explorer', {
    tag: [_test_setup_1.tags.WEB, _test_setup_1.tags.DATA_EXPLORER, _test_setup_1.tags.DUCK_DB, _test_setup_1.tags.WIN]
}, () => {
    ...
    testCases.forEach(({ target, file, copyValue }) => {
        (0, _test_setup_1.test)(`Verify can open and view data with large ${target} file`, { tag: [_test_setup_1.tags.PERFORMANCE] }, async function ({ app, openDataFile, metric }) {
    ...
    plainTextTestCases.forEach(({ target, file, searchString }) => {
        (0, _test_setup_1.test)(`Verify can open ${target} file as plaintext`, { tag: [infra_1.TestTags.EDITOR_ACTION_BAR] }, async function ({ app, openDataFile }) {
            const { dataExplorer, editors } = app.workbench;
            await openDataFile((0, path_1.join)(`data-files/flights/${file}`));
    ...
    (0, _test_setup_1.test)(`Verify can open parquet decimal data`, async function ({ app, openDataFile }) {
        const { editors, dataExplorer, clipboard } = app.workbench;
...
//# sourceMappingURL=data-explorer-headless.test.js.map