...
_test_setup_1.test.describe('Data Explorer: Summary Panel', { tag: [_test_setup_1.tags.WIN, _test_setup_1.tags.WEB, _test_setup_1.tags.DATA_EXPLORER] }, () => {
    _test_setup_1.test.beforeEach(async function ({ openDataFile }) {
        await openDataFile((0, path_1.join)('data-files', 'small_file.csv'));
    });
    _test_setup_1.test.afterEach(async function ({ hotKeys }) {
        await hotKeys.closeAllEditors();
    });
    (0, _test_setup_1.test)('Summary Panel: Search', async function ({ app }) {
    ...
    (0, _test_setup_1.test)('Summary Panel: Sort', async function ({ app }) {
        const { dataExplorer } = app.workbench;
        // view data in data explorer
        await dataExplorer.maximize();
        await dataExplorer.waitForIdle();
    ...
    (0, _test_setup_1.test)('Summary Panel: Behavior with Pins', async function ({ app }) {
        const { dataExplorer } = app.workbench;
        // view data in data explorer
        await dataExplorer.maximize();
        await dataExplorer.waitForIdle();
    ...
    (0, _test_setup_1.test)('Summary Panel: Behavior on right', async function ({ app }) {
        const { dataExplorer } = app.workbench;
        // view data and show summary panel on right side
        await dataExplorer.maximize();
        await dataExplorer.waitForIdle();
        await dataExplorer.summaryPanel.show('right');
        await (0, test_1.expect)(dataExplorer.summaryPanel.summaryPanel).toHaveClass(/.*right-column.*/);
        ...
        (0, test_1.expect)(searchBarY).toBeLessThan(firstColumnY);
    });
});
//# sourceMappingURL=data-explorer-summary-panel.test.js.map