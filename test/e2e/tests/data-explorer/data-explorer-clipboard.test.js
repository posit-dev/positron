...
const testCases = [
    { env: 'R', rowIndexOffset: 1, data: 'df <- read.csv("data-files/small_file.csv")', tags: [_test_setup_1.tags.WIN] },
    { env: 'DuckDB', rowIndexOffset: 0, data: 'data-files/small_file.csv', tags: [_test_setup_1.tags.WIN] },
    { env: 'Polars', rowIndexOffset: 0, data: 'import polars as pl; df = pl.read_csv("data-files/small_file.csv")', tags: [_test_setup_1.tags.WIN] },
...
for (const { env, data, rowIndexOffset: indexOffset, tags: testTags = [] } of testCases) {
    _test_setup_1.test.describe('Data Explorer: Copy/Paste', { tag: [_test_setup_1.tags.WEB, _test_setup_1.tags.DATA_EXPLORER, ...testTags] }, () => {
        _test_setup_1.test.beforeEach(async function ({ app, openDataFile, hotKeys }) {
            const { dataExplorer, console, sessions, variables } = app.workbench;
            if (env === 'DuckDB') {
                await openDataFile((0, path_1.join)(data));
            }
            else {
        ...
        (0, _test_setup_1.test)(`${env} - Copy and paste works on cells, rows, columns, and ranges of unsorted data`, async function ({ app }) {
        ...
        (0, _test_setup_1.test)(`${env} - Copy and paste works on cells, rows, columns, and ranges of sorted data`, async function ({ app }) {
            const { dataExplorer, clipboard } = app.workbench;
        ...
        (0, _test_setup_1.test)(`${env} - Copy and paste of ranges works with pinned data`, async function ({ app }) {
            const { dataExplorer, clipboard } = app.workbench;
            // pin column 4
...
//# sourceMappingURL=data-explorer-clipboard.test.js.map