...
exports.availableRuntimes = exports.SessionQuickPick = exports.Sessions = exports.DISCONNECTED_STATUS_ICON = exports.IDLE_STATUS_ICON = exports.ACTIVE_STATUS_ICON = void 0;
...
exports.ACTIVE_STATUS_ICON = '.codicon-positron-runtime-status-active';
exports.IDLE_STATUS_ICON = '.codicon-positron-runtime-status-idle';
exports.DISCONNECTED_STATUS_ICON = '.codicon-positron-runtime-status-disconnected';
...
class Sessions {
    ...
    async start(sessions, options) {
        const { triggerMode = 'hotkey', reuse = true, } = options || {};
        // convert input to array for unified processing
        const sessionsToCreate = (Array.isArray(sessions) ? sessions : [sessions]);
        const results = [];
        // Helper to create a new session and fetch metadata
        const createSession = async (session) => {
            // Get a fresh session object with current environment values
            const sessionTemplate = exports.availableRuntimes[session];
            const newSession = {
                ...sessionTemplate,
                waitForReady: true,
                triggerMode,
        ...
        if (reuse) {
            ...
            for (const session of sessionsToCreate) {
                const sessionName = exports.availableRuntimes[session].name;
        ...
        return (Array.isArray(sessions) ? results : results[0]);
    }
    /**
    ...
    async delete(sessionId) {
        await test_1.default.step(`Delete session: ${sessionId}`, async () => {
            await (0, test_1.expect)(async () => {
                ...
                await (0, test_1.expect)(this.page.getByText('Shutting down')).not.toBeVisible();
                await (0, test_1.expect)(this.consoleInstance(sessionId)).not.toBeVisible();
     ...
     */
    async restart(sessionIdOrName, options) {
        const { waitForIdle = true, clearConsole = true, clickModalButton = '' } = options || {};
        await test_1.default.step(`Restart session: ${sessionIdOrName}`, async () => {
            await this.console.focus();
            if (await this.getSessionCount() > 1) {
            ...
            if (waitForIdle) {
                await (0, test_1.expect)(this.page.getByText('restarting.')).not.toBeVisible({ timeout: 90000 });
                await (0, test_1.expect)(this.page.locator('.console-instance[style*="z-index: auto"]').getByText('restarted.')).toBeVisible({ timeout: 90000 });
     ...
     */
    async selectMetadataOption(menuItem) {
        await this.console.focus();
        await this.metadataButton.click();
        await this.metadataDialog.getByText(menuItem).click();
        await (0, test_1.expect)(this.page.getByRole('tab', { name: 'Output' })).toHaveClass(/.*checked.*/);
    }
    /**
     * Action: Delete all disconnected sessions
     */
    async deleteDisconnectedSessions() {
        await test_1.default.step('Delete all disconnected sessions', async () => {
     ...
     */
    async deleteAll() {
        await test_1.default.step('Delete all sessions', async () => {
            const sessionIds = await this.getAllSessionIds();
            // Delete all sessions
            ...
            if (this.code.driver.currentPage.url().includes('8080')) {
                try {
                    await this.page.getByRole('button', { name: 'Delete Session' }).click({ timeout: 1000 });
                }
                catch (error) { }
            }
            else {
                await (0, test_1.expect)(this.page.getByText('There is no session running.')).toBeVisible();
            }
        });
    }
    /**
    ...
    async clearConsoleAllSessions() {
        await test_1.default.step('Clear all sessions', async () => {
            const sessionIds = await this.getAllSessionIds();
     ...
     */
    async setSessionDividerAboveBottom(distanceFromBottom = 100) {
     ...
     */
    async resizeSessionList(options) {
        await test_1.default.step(`Resize session list: ${options}`, async () => {
            const { x, y } = options;
     ...
     * @param sessionIdOrName - the id or name of the session
     */
    async select(sessionIdOrName, waitForSessionIdle = false) {
        await test_1.default.step(`Select session: ${sessionIdOrName}`, async () => {
            await this.console.focus();
            const sessionTab = this.getSessionTab(sessionIdOrName);
            if (waitForSessionIdle) {
                await (0, test_1.expect)(this.idleStatus(sessionTab)).toBeVisible();
            }
    ...
    async getSessionCount() {
        return await test_1.default.step('Get console session count', async () => {
            await this.console.focus();
    ...
    async startAndSkipMetadata(options) {
        ...
        const { language, version = language === 'Python' ? getDesiredPython() : getDesiredR(), waitForReady = true, triggerMode = 'hotkey', } = options;
        return await test_1.default.step(`Start session via ${triggerMode}: ${language} ${version} ${options.disambiguator}`, async () => {
            ...
            if (waitForReady) {
                await (0, test_1.expect)(this.console.activeConsole.getByText(/started/)).toBeVisible({ timeout: 90000 });
            }
            ...
            if (options.disambiguator) {
                ...
                await (0, test_1.expect)(async () => {
                    ...
                    targetSession = allSessions.find(s => s.name.toLowerCase().includes(options.disambiguator.toLowerCase()));
                    (0, test_1.expect)(targetSession).toBeDefined();
    ...
    async expectNoStartUpMessaging() {
        await test_1.default.step('Wait runtimes to finish loading', async () => {
            await (0, test_1.expect)(this.code.driver.currentPage.locator('[id="workbench.parts.titlebar"]')).toBeVisible({ timeout: 30000 });
            await this.console.focus();
            await this.code.driver.currentPage.mouse.move(0, 0);
            await (0, test_1.expect)(this.page.locator('text=/^Waiting for extensions|^Starting|^Preparing|Reconnecting|^Reactivating|^Discovering( \\w+)? interpreters|starting\\.$/i')).toHaveCount(0, { timeout: 90000 });
        });
    }
    /**
    ...
    async getAllSessionIds() {
        ...
        return sessionIds.filter(sessionId => sessionId !== null);
    }
    /**
     ...
     */
    async getAllSessionIdsAndNames() {
        return await test_1.default.step('Get all session IDs and names', async () => {
    ...
    async getCurrentSessionId() {
        return await test_1.default.step('Get current session ID', async () => {
    ...
    async getMetadata(sessionId) {
        return await test_1.default.step(`Get metadata for: ${sessionId ?? 'current session'}`, async () => {
     ...
     */
    async extractMetadataFromDialog() {
        let metadata;
        await test_1.default.step('Extract metadata from dialog', async () => {
            await (0, test_1.expect)(async () => {
    ...
    async getActiveSessions() {
        ...
        const activeSessions = (await Promise.all(allSessionTabs.map(async (session) => {
     ...
     */
    async getIconStatus(sessionIdOrName) {
        const session = this.getSessionTab(sessionIdOrName);
        if (await this.activeStatus(session).isVisible()) {
            return 'active';
        }
        if (await this.idleStatus(session).isVisible()) {
            return 'idle';
        }
        if (await this.disconnectedStatus(session).isVisible()) {
            return 'disconnected';
        }
        return 'unknown';
    }
    ...
    async rename(oldName, newName) {
        await test_1.default.step(`Rename session: ${oldName} to ${newName}`, async () => {
     ...
     */
    async renameViaUI(sessionId, newName) {
        await test_1.default.step(`Rename session: ${sessionId} to ${newName}`, async () => {
            await this.console.focus();
            ...
            // input the new name
            await (0, test_1.expect)(sessionTab.getByRole('textbox')).toBeVisible();
    ...
    */
    async openMetadataDialog() {
        await (0, test_1.expect)(async () => {
            ...
            await (0, test_1.expect)(this.metadataDialog).toBeVisible();
     ...
     */
    async expectStatusToBe(sessionIdOrName, expectedStatus, options) {
        const timeout = options?.timeout || 30000;
        await test_1.default.step(`Verify ${sessionIdOrName} session status: ${expectedStatus}`, async () => {
            const sessionCount = await this.getSessionCount();
            if (sessionCount > 1) {
                ...
                await (0, test_1.expect)(sessionTab.locator(statusClass)).toBeVisible({ timeout });
            }
            else if (sessionCount === 1) {
                // get status from metadata dialog because there is no tab list view
                await this.openMetadataDialog();
                await (0, test_1.expect)(this.metadataDialog.getByText(`State: ${expectedStatus}`)).toBeVisible({ timeout });
                await this.page.keyboard.press('Escape');
            }
            else {
     ...
     */
    async expectSessionNameToBe(sessionId, expectedName) {
        await test_1.default.step(`Verify session name: ${sessionId} is ${expectedName}`, async () => {
            const sessionTab = this.getSessionTab(sessionId);
            const tabHeader = sessionTab.locator('.tab-header');
            await (0, test_1.expect)(tabHeader).toHaveText(expectedName);
        });
    }
    /**
     ...
     */
    async expectMetadataToBe(session) {
        await test_1.default.step(`Verify ${session.name} metadata: ${session.state}, ${session.path}`, async () => {
            await this.getSessionTab(session.id).click();
            await this.openMetadataDialog();
            // Verify metadata
            await (0, test_1.expect)(this.metadataDialog.getByTestId('session-name')).toContainText(session.name);
            await (0, test_1.expect)(this.metadataDialog.getByTestId('session-id')).toContainText(session.id);
            await (0, test_1.expect)(this.metadataDialog.getByTestId('session-state')).toContainText(session.state);
            await (0, test_1.expect)(this.metadataDialog.getByTestId('session-path')).toContainText(session.path);
            await (0, test_1.expect)(this.metadataDialog.getByTestId('session-source')).toContainText(session.source);
            await this.page.keyboard.press('Escape');
            const language = session.name.split(' ')[0].trim();
            await this.selectMetadataOption('Show Supervisor Output Channel');
            await (0, test_1.expect)(this.outputChannel).toHaveValue(`${language} Supervisor`);
            // Verify Output Channel
            await this.selectMetadataOption('Show Kernel Output Channel');
            await (0, test_1.expect)(this.outputChannel).toHaveValue(`${language} Kernel`);
            // Verify LSP Output Channel
            await this.selectMetadataOption('Show LSP Output Channel');
            await (0, test_1.expect)(this.outputChannel).toHaveValue(`${language} Language Server`);
     ...
     */
    async expectSessionPickerToBe(runtimeName, timeout = 15000) {
        await test_1.default.step(`Verify runtime is selected: ${runtimeName}`, async () => {
            const normalizedRuntimeName = runtimeName.replace(/-\s\d+$/, '').trim();
            await (0, test_1.expect)(this.sessionPicker).toHaveText(normalizedRuntimeName, { timeout });
        });
    }
    /**
     ...
     */
    async expectSessionCountToBe(count, sessionType = 'all') {
        await test_1.default.step(`Verify session count: ${count}`, async () => {
            await (0, test_1.expect)(async () => {
                if (sessionType === 'active') {
                    const activeSessionsFromConsole = await this.getActiveSessions();
                    (0, test_1.expect)(activeSessionsFromConsole).toHaveLength(count);
                }
                else {
                    if (count === 0) {
                        await (0, test_1.expect)(this.sessionTabs).not.toBeVisible();
                        await (0, test_1.expect)(this.metadataButton).not.toBeVisible();
                    }
                    else if (count === 1) {
                        await (0, test_1.expect)(this.sessionTabs).not.toBeVisible();
                        await (0, test_1.expect)(this.metadataButton).toBeVisible();
                    }
                    else {
                        await (0, test_1.expect)(this.sessionTabs).toHaveCount(count);
                    }
                }
    ...
    async expectActiveSessionListsToMatch() {
        await test_1.default.step('Verify active sessions match between console and session picker', async () => {
            await (0, test_1.expect)(async () => {
                const activeSessionsFromConsole = await this.getActiveSessions();
                const activeSessionsFromPicker = await this.quickPick.getActiveSessions();
                (0, test_1.expect)(activeSessionsFromConsole).toStrictEqual(activeSessionsFromPicker);
     ...
     */
    async expectSessionListToBeScrollable(options = {}) {
        ...
        (0, test_1.expect)(isHorizontallyScrollable).toBe(horizontal);
        (0, test_1.expect)(isVerticallyScrollable).toBe(vertical);
    }
    /**
     * Verify: all sessions are "ready" (idle or disconnected)
     */
    async expectAllSessionsToBeReady({ timeout = 15000 } = {}) {
        await test_1.default.step('Expect all sessions to be ready', async () => {
            await this.expectNoStartUpMessaging();
            await (0, test_1.expect)(this.activeStatusIcon).toHaveCount(0, { timeout });
        });
    }
    /**
    ...
    async expectStartNewSessionMenuToBeVisible() {
        await (0, test_1.expect)(this.quickPick.allSessionsMenu).toBeVisible();
    }
    /**
     ...
     */
    async expectSessionQuickPickToContainInRelativeOrder(sessionList) {
        ...
        for (const { session } of sessionList) {
            const nextIndex = actualEntries.findIndex((entry, i) => i >= currentIndex &&
                entry.name === session.name &&
                entry.path === session.path);
            (0, test_1.expect)(nextIndex).not.toBe(-1);
 ...
 */
class SessionQuickPick {
    code;
    sessions;
    get quickInputTitleBar() { return this.code.driver.currentPage.locator('.quick-input-titlebar'); }
    get sessionQuickMenu() { return this.quickInputTitleBar.getByText(SESSION_QUICK_MENU_PATTERN); }
    get allSessionsMenu() { return this.quickInputTitleBar.getByText(START_NEW_CONSOLE_SESSION_PATTERN); }
    constructor(code, sessions) {
        this.code = code;
    ...
    async openSessionQuickPickMenu(viewAllRuntimes = true) {
        await test_1.default.step('Open session quickpick menu', async () => {
            ...
            await (0, test_1.expect)(async () => {
                ...
                if (viewAllRuntimes) {
                    ...
                    await this.code.driver.currentPage.keyboard.press('Enter');
                    await (0, test_1.expect)(this.code.driver.currentPage.getByText(START_NEW_CONSOLE_SESSION_PATTERN)).toBeVisible({ timeout: 1000 });
                }
    ...
    async closeSessionQuickPickMenu() {
        await test_1.default.step('Close session quickpick menu', async () => {
            if (await this.sessionQuickMenu.isVisible()) {
                await this.code.driver.currentPage.keyboard.press('Escape');
                await (0, test_1.expect)(this.sessionQuickMenu).not.toBeVisible();
            }
        });
    }
    // --- Helpers ---
     ...
     */
    async getActiveSessions() {
        return await test_1.default.step('Get active sessions from session picker', async () => {
            ...
            const activeSessions = await Promise.all(allSessions.map(async (element) => {
     ...
     */
    async parseRuntimeName(runtimeName) {
        if (!runtimeName) {
...
const createPythonSessionHidden = () => ({
