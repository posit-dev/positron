...
/*
 * Internal shared base for the notebooks page object. Holds the page-object
 * dependencies, the webview frame locator, and the editor-agnostic notebook
 * commands that `PositronNotebooks` builds on. It exists only so these
 * constructor-set members are initialized before `PositronNotebooks`'s
 * `this.code`-based field locators read them (see `useDefineForClassFields`).
 * Not exported and not registered -- `PositronNotebooks` is the one notebooks POM.
 */
class NotebooksBase {
	protected code: Code;
