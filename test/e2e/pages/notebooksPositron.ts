...
export class PositronNotebooks extends Notebooks {
	 ...
	 * @param options.editMode - Whether to enter edit mode after selection.
	 * @param options.addToSelection - If true, Cmd/Ctrl+click to add to existing selection.
	 */
	async selectCellAtIndex(
		cellIndex: number,
		{ editMode = undefined, addToSelection = false }: { editMode?: boolean; addToSelection?: boolean } = {}
	): Promise<void> {
		await test.step(`Select cell at index: ${cellIndex}, edit mode: ${editMode}, addToSelection: ${addToSelection}`, async () => {
			const cell = this.cell.nth(cellIndex);
			const cellType = await this.getCellType(cellIndex);
	...
	async expectCellContentAtIndexToBe(cellIndex: number, expectedContent: string | string[]): Promise<void> {
		await test.step(`Expect cell ${cellIndex} content to be: ${expectedContent}`, async () => {
			// Wrap in expect.toPass() so the entire read-and-assert cycle retries
			// until content matches. getCellContent uses allTextContents() which is
			// a non-retrying snapshot -- without this, we race Monaco rendering.
			await expect(async () => {
