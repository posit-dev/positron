 ...
 */
class TestExplorer extends explorer_1.Explorer {
    /**
    ...
    async verifyTestFilesExist(files) {
        const projectFiles = this.code.driver.currentPage.locator('.test-explorer');
        for (let i = 0; i < files.length; i++) {
            await (0, test_1.expect)(projectFiles.getByLabel(files[i])).toBeVisible({ timeout: 3000 });
        }
    }
    /**
