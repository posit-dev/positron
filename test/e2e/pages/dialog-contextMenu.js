...
class ContextMenu {
    ...
    isNativeMenu;
    get contextMenu() { return this.page.locator('.monaco-menu'); }
    getContextMenuItem(label) { return this.contextMenu.getByRole('menuitem', { name: label }); }
    getContextMenuCheckboxItem(label) { return this.contextMenu.getByRole('menuitemcheckbox', { name: label }); }
    constructor(code) {
        this.code = code;
     ...
     */
    async triggerMenu(menuTrigger, menuTriggerButton = 'left') {
        if (this.isNativeMenu && this.code.electronApp) {
            ...
            try {
                await (0, test_1.expect)(async () => {
                    ...
                    (0, test_1.expect)(nativeResult).toBeDefined();
                }).toPass({ timeout: 5000 });
            }
            catch {
            ...
            try {
                await (0, test_1.expect)(async () => {
                    ...
                    if (!visible) {
                        console.warn('[contextMenu] web menu did not appear');
                    }
     ...
     */
    async triggerAndClick({ menuTrigger, menuItemLabel, menuItemType = 'menuitem', menuTriggerButton = 'left' }) {
        await test_1.default.step(`Trigger context menu and click '${menuItemLabel}'`, async () => {
            if (this.isNativeMenu) {
                ...
                await (0, test_1.expect)(async () => {
                    ...
                    // Trigger the menu
                    await menuTrigger.click({ button: menuTriggerButton });
                    await (0, test_1.expect)(this.contextMenu).toBeVisible({ timeout: 2000 });
    ...
    async showContextMenu(trigger) {
        ...
        try {
            ...
            const shownPromise = this.code.electronApp.evaluate(({ app }) => {
                return new Promise((resolve) => {
                    const listener = (...args) => {
                        app.removeListener('e2e:contextMenuShown', listener);
                        resolve(args);
                    };
                    app.addListener('e2e:contextMenuShown', listener);
                });
            });
            if (!shownPromise) {
                return undefined;
     ...
     */
    async selectContextMenuItem(contextMenuId, label) {
     ...
     * @param menuItemLabel The label of the menu item to click
     */
    async nativeMenuTriggerAndClick({ menuTrigger, menuItemLabel, menuTriggerButton = 'left' }) {
    ...
    async triggerAndVerifyMenuItems({ menuTrigger, menuTriggerButton = 'left', menuItemStates }) {
        ...
        if (this.isNativeMenu) {
            ...
            for (const expectedItem of menuItemStates) {
                const menuItem = menuItems.items.find(item => typeof expectedItem.label === 'string'
                    ? item.label.includes(expectedItem.label)
                    : expectedItem.label.test(item.label));
                if (!menuItem) {
            ...
            for (const { label: menuLabel, visible, enabled } of menuItemStates) {
                ...
                if (typeof visible === 'boolean') {
                    visible
                        ? await (0, test_1.expect)(menuItem).toBeVisible({ timeout: 2000 })
                        : await (0, test_1.expect)(menuItem).not.toBeVisible({ timeout: 2000 });
                }
                // verify enabled state
                if (typeof enabled === 'boolean') {
                    enabled
                        ? await (0, test_1.expect)(menuItem).toBeEnabled({ timeout: 2000 })
                        : await (0, test_1.expect)(menuItem).toBeDisabled({ timeout: 2000 });
                }
                // verify checked state
...
//# sourceMappingURL=dialog-contextMenu.js.map