...
test.describe('Release Screenshots - Open Folder', () => {
	 ...
	 * dropdown menu expanded, highlighting the "open a folder" entry points:
	 * top-left Open button, and the top-right folder dropdown + its menu.
	 */
	test('Release Screenshot - open-folder.png', async ({ app, page, hotKeys, r }) => {
		...
		// Open the Welcome editor explicitly, so the shot shows what a session
		// with no folder open actually looks like.
		await quickaccess.runCommand('workbench.action.openWalkthrough');
		await expect(page.locator('.positron-welcome-page').first()).toBeVisible({ timeout: 15000 });
		...
		// Crop to the top portion (title bar + action bar + the dropdown menu) so
		// the console/panel area is excluded.
		...
		if (!titleBox || !menuBox || !workbench) {
			throw new Error('Could not measure crop region');
		}
		const bottom = Math.ceil(menuBox.y + menuBox.height + 16);
