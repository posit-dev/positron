...
function generateRustEvent(event: PositronEventDefinition): string {

	const lines: string[] = [];

	const comment = event.comment.trimEnd().replace(/(^|\n)/g, '$1/// ');
	lines.push(comment);
	lines.push(`#[positron::event("${snake(event.name)}")]`);
...
function generateRustPositronEventEnum(): string {
...
function generateRustClientEventHelper(): string {
...
function updateRustEventsFile(rustEventsFile: string): void {
...
function updateRustClientEventsFile(path: string): void {
...
function generatePythonEvent(event: PositronEventDefinition): string {

	const lines: string[] = [];

	lines.push(`class ${event.name}Event(BaseFrontendEvent):`);
	lines.push('    """');
	...
	for (const param of event.params) {
		lines.push(`    # ${param.comment}`);
		lines.push(`    ${param.name}: ${pythonTypesMap[param.type]}`);
		lines.push('');
	}

	lines.push(`    name: str = "${snake(event.name)}"`);

	return lines.join('\n');

}

function updatePythonEventsFile(pythonEventsFile: string): void {

	// Generate event definitions for Python
	const pythonEvents = events.map(generatePythonEvent);
...
class BaseFrontendEvent(BaseModel):
    """
    Event submitted to the frontend service.
    """
    ...
    class Config:
        fields = {
            "name": {
                # If the unserialized event provides a \`name\`, it must match the default value.
                "const": True,
                # Exclude \`name\` from the serialized event.
                "exclude": True,
                "description": "Name of the event.",
            }
        }`;

	pythonEvents.unshift(header);

	// Add two empty lines between each class.
	let result = pythonEvents.join('\n\n\n');

	// End the file with a newline.
	result += '\n';

	writeFileSync(pythonEventsFile, result);

}

function generateLanguageRuntimeEventTypeEnum(): string {
...
function generateLanguageRuntimeEventDefinitions(): string {
...
function generateLanguageRuntimeEventsFile(languageRuntimeEventsFile: string): void {
...
const pythonEventsFile = 'extensions/positron-python/pythonFiles/positron/frontend_events.py';
updatePythonEventsFile(pythonEventsFile);
