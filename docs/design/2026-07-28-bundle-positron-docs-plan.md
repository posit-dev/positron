...
10. **The `workbench` profile emits relative `llms.txt` links, and one `DOCS_BASE_URL` is correct for
    both profiles.** `_quarto-workbench.yml` sets no `site-url` (only `_quarto-positron.yml` and
    `_quarto-dailies.yml` do), which raises the question of what its links look like and whether
    `docsBaseUrl` should differ per profile. Both halves are settled, verified against the pinned
    Quarto 1.9.23 rather than reasoned about:
    - **Link shape.** `llms.txt` is generated by Quarto's own `website: llms-txt: true`. Rendering the
      same fixture twice, once with `site-url` and once without, gives
      `- [Home](https://positron.posit.co/index.llms.md)` versus `- [Home](index.llms.md)`. With no
      `site-url` Quarto emits **plain relative** links - not root-relative. So the `sed` rewrite is a
      harmless no-op for `workbench`, and every guard passes. Confirmed by running the real script over
      workbench-shaped input. (Note the local Quarto must be 1.9.23 to reproduce this: 1.6.11 rejects
      the `llms-txt` key outright.)
    - **`docsBaseUrl`.** The workbench footer points at `docs.posit.co/ide/server-pro/`, which suggests
      a different base is needed, but that is the Posit Workbench *product* documentation - a separate
      set, not where these pages live. `_quarto-positron.yml` has no `render:` filter and
      `_quarto-workbench.yml`'s is exclusions only, so the workbench pages are a strict **subset** of
      the positron site's. Every page in the workbench bundle therefore resolves at
      `https://positron.posit.co/<page>.html`, which is exactly what spec line "join `docsBaseUrl` with
      the relative path and swap `.llms.md` for `.html`" needs. One hardcoded value is right for both.
