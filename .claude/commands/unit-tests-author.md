...
4. For extension files that import `vscode` or `positron`: check if the import is for types/enums only (can still be unit tested with stubs) or genuinely needs extension host APIs (recommend `npm run test-extension` instead).
...
7. **After all tests are written**, run the full suite for the affected area:
   ```bash
   ./scripts/test.sh --runGlob '<glob>.test.js'
   ```
   Report the total and confirm no regressions.
