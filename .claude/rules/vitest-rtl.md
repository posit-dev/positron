...
The matchers register at runtime via `src/vs/test/vitest/setup.ts` (loaded by Vitest's `setupFiles`), so `npx vitest run` works without any per-file setup. For the IDE to recognize them, [`src/vs/test/vitest/reactTestingLibrary.tsx`](../../src/vs/test/vitest/reactTestingLibrary.tsx) carries a `/// <reference types="@testing-library/jest-dom/vitest" />` directive — tests that import `setupRTLRenderer` from there pick the matchers up transitively. A React test that uses jest-dom matchers but doesn't import the helper needs the same directive on its own first lines; see [`textInputActions.vitest.tsx`](../../src/vs/workbench/browser/actions/test/browser/textInputActions.vitest.tsx) for an example.
