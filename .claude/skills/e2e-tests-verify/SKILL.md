...
- **Do NOT invent runner syntax that doesn't exist.** There is no `captureAs`, `$ref.id`, `$pySession`, or variable interpolation syntax in `/run-plan`. If you need a return value (like a session ID), split into two `/run-plan` calls: the first runs `sessions.start` and returns metadata in the response JSON, then read the ID from the response and pass it as a literal string in the second `/run-plan`. The runner CAN return values -- you just read them from the response, not via in-plan variable references.
- **Do NOT report hallucinated limitations as rough edges.** If a step fails because you used syntax that doesn't exist, that's your error, not a runner limitation. Only report rough edges for genuine UX issues you observed.
