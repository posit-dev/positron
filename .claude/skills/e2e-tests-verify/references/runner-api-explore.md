...
```bash
PORT=$(cat /tmp/explore-runner-port-MMDD-N-SLUG) && curl -s -X POST "http://localhost:${PORT}/pom" \
...
```bash
PORT=$(cat /tmp/explore-runner-port-MMDD-N-SLUG) && curl -s -X POST "http://localhost:${PORT}/action" \
  -H 'Content-Type: application/json' \
...
```bash
PORT=$(cat /tmp/explore-runner-port-MMDD-N-SLUG) && curl -s -X POST "http://localhost:${PORT}/batch" \
  -H 'Content-Type: application/json' \
