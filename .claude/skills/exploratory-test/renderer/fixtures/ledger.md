...
## S02 - Edge values (all-NaN, inf, all-null string, object, complex)
Status: pass
Result: Every expected sparkline loads, no stuck placeholders

Steps:
1. Create `edge` with all-NaN, inf, all-null string, object and complex columns.
2. Run `%view edge`.
3. VERIFY Every column shows its sparkline and no loading dots remain. -> PASS
   Evidence: 02-edge-open.png
...
## S05 - polars frame
Status: pass
Result: Same summaries as pandas

Steps:
1. Run `pl_df = pl.from_pandas(df)`, then `%view pl_df`.
2. VERIFY The summaries match the pandas frame's. -> PASS
   Evidence: 05-polars.png

## S06 - DuckDB relation over Parquet
Status: pass
Result: Summaries load for every column

Preconditions:
- `trips.parquet` on disk | | Write 1,000 rows of `df` with `df.to_parquet("trips.parquet")`.

Steps:
1. Run `rel = duckdb.read_parquet("trips.parquet")`, then `%view rel`.
2. VERIFY Every column gets a summary. -> PASS
   Evidence: 06-duckdb.png
...
3. VERIFY The column summary loads. -> FAIL - Finding 1
   ...
   Log: get_column_profiles timed out after 10 seconds (Renderer, 2x)
