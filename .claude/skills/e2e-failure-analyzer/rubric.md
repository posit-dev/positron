...
- **DOM presence + console digest (appended to the trace timeline).** Two derived sections accompany the timeline when the trace carries the data:
  - **DOM presence** answers, for the failing selector's target, whether it *ever* entered the DOM across all frame snapshots. `NEVER present in any snapshot` means the element genuinely never rendered -- a **product open-path bug**, NOT a render-then-dismiss and NOT a stale selector. (The moment-of-failure error-context snapshot is a single instant and cannot tell "never appeared" from "appeared then vanished"; this section can.) `present in N/M snapshots (t=..)` with a visibility/timeout error points instead to a timing/dismiss race.
