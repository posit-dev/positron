...
- **Sibling tests in the same file ("Other tests in this file: ..." in the input).** Before blaming setup/fixtures, check whether a sibling test PASSED. If the failing test depends on a fixture created in `beforeAll`/global setup and a sibling passed using that same fixture, setup succeeded and the fixture WAS provisioned -- so the failure is something mutating or removing the fixture mid-run, NOT "setup never ran." A "file not found" with a green sibling is a lifecycle/race, not a provisioning failure.
