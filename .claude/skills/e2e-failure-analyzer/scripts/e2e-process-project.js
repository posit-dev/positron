...
/**
 * Mine the attached log bundle for the failure window. Delegates to
 ...
 * an info-level success line (e.g. "Fetched 11 models from API", which refutes
 * an "external dependency broke" theory) and a log going silent exactly when the
 * UI should have appeared.
 */
...
for (const testId of failedTestIds) {
	...
	// Mine the attached log bundle for the failure window. One bundle per test is
	// enough; the last attempt's logs are the most relevant. Runs AFTER the trace
	// loop because the window and wall-clock anchor come out of the trace -- the
	// logs cannot be time-sliced without them.
	let logExcerpt = null;
	...
	if (logsAtt?.path) {
		...
		if (logsZipPath) {
			...
			logExcerpt = mineLogBundle(logsZipPath, lastTrace, testInfo?.file);
		}
	}
