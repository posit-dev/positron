...
async function transpile(outDir: string, excludeTests: boolean): Promise<void> {
	...
	// --- Start Positron ---
	// Also include .tsx files for transpilation (JSX support)
...
async function watch(): Promise<void> {
		...
		try {
			...
			if (tsFiles.length > 0) {
				...
				await Promise.all(tsFiles.map(srcPath => {
					...
					// --- Start Positron ---
					// Support .tsx files in watch mode
	...
	watchStream.on('data', (file: { path: string }) => {
		// --- Start Positron ---
		// Add .tsx to the watch list
		if ((file.path.endsWith('.ts') || file.path.endsWith('.tsx')) && !file.path.endsWith('.d.ts')) {
			// --- End Positron ---
