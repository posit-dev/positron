...
export async function transpileFile(srcPath: string, destPath: string): Promise<void> {
	...
	// --- Start Positron ---
	// Determine loader based on file extension (.ts vs .tsx) to enable JSX support in .tsx files
	const loader = srcPath.endsWith('.tsx') ? 'tsx' : 'ts';
...
export async function applyIncrementalClientChanges(repoRoot: string, outDir: string, changedPaths: readonly string[]): Promise<void> {
	...
	const operations = await mapWithConcurrency([...destinations], MAX_CONCURRENT_FILE_OPERATIONS, async destination => {
		...
		if (destination.endsWith('.js')) {
			// --- Start Positron ---
			// Try .ts then .tsx so React sources are transpiled (not treated as copies).
			for (const ext of ['.ts', '.tsx']) {
				const typeScriptSource = path.join(repoRoot, 'src', destination.slice(0, -'.js'.length) + ext);
...
export function getOutputRelativePath(sourceRelativePath: string): string {
	// --- Start Positron ---
	// Map .tsx as well as .ts to .js so React sources build.
	return /\.tsx?$/.test(sourceRelativePath) && !sourceRelativePath.endsWith('.d.ts')
