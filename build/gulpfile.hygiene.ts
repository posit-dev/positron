...
import { checkCopilotEnginesVersion, checkNoNewJavaScriptFiles, checkNoReintroducedDependencies, hygiene } from './hygiene.ts';
...
const checkPackageJSONTask = task.define('check-package-json', () => {
	return gulp.src('package.json').pipe(
		es.through(function () {
			...
			const removedDepsError = checkNoReintroducedDependencies(repoRoot);
			if (removedDepsError) {
				this.emit('error', removedDepsError);
			}
		})
	);
});
