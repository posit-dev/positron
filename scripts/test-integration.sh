...
# --- Start Positron ---
# Append $LINUX_EXTRA_ARGS for Positron CI runs (sandbox/no-xvfb plumbing).
API_TESTS_EXTRA_ARGS="--disable-telemetry --disable-experiments --skip-welcome --skip-release-notes --crash-reporter-directory=$VSCODECRASHDIR --logsPath=$VSCODELOGSDIR --no-cached-data --disable-updates --use-inmemory-secretstorage --disable-extensions --disable-workspace-trust --user-data-dir=$VSCODEUSERDATADIR $LINUX_EXTRA_ARGS"
# --- End Positron ---
...
# --- Start Positron ---
# Colorize tests disabled — see https://github.com/posit-dev/positron/issues/7349
...
echo
echo "### Positron SQL tests"
echo
run_extension_suite npm run test-extension -- -l positron-sql
kill_app
