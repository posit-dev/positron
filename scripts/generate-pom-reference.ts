 ...
 * signatures and leading JSDoc summaries.
 *
 ...
 *
 * Output is git-ignored (see .gitignore) -- it's disposable and meant to be regenerated on
 * demand, not committed. Run via: npm run e2e-gen-pom-reference
 */
...
function renderPomFile(entry: PomEntry): string {
	...
	lines.push('');
	lines.push(`Auto-generated by \`scripts/generate-pom-reference.ts\` from \`test/e2e/${entry.relPath}\`. Not committed -- regenerate with \`npm run e2e-gen-pom-reference\` any time you need it.`);
	lines.push('');
...
function renderIndex(entries: PomEntry[]): string {
	...
	lines.push('');
	lines.push(`Auto-generated by \`scripts/generate-pom-reference.ts\`. Not committed -- this whole directory is git-ignored and regenerated fresh each time via \`npm run e2e-gen-pom-reference\`.`);
	lines.push('');
