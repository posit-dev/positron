...
- **Desktop / GUI is via VNC, always on.** `post-start` starts a window manager (fluxbox) +
  x11vnc on the headless display, so you can connect **any time** — no task needed first. Open a
  VNC viewer to `vnc://localhost:5900`, password **`positron`** (macOS: Finder → Cmd+K → Screen
  Sharing; not a browser — VNC isn't HTTP). Anything that runs headed (the Electron app, or a
  headed `e2e-chromium`/`e2e-firefox` browser) shows up on that desktop. The browser server
  (`:8080`) is the lighter option for just poking at Positron. No noVNC in the image, so Windows
  users would need a VNC client.
