...
#   - the data volumes: root + test/e2e node_modules, .build, postgres-data
...
if [ -z "$anchor" ]; then
  ...
  vols="$(docker volume ls -q | grep -E "^${prefix}(positron-node-modules|positron-e2e-node-modules|positron-build|postgres-data)$" || true)"
fi
