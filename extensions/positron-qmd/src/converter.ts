...
/**
 * Find the start of the code fence line before a code block.
 * Uses the code content to search for the fence in the source.
 */
function findFenceStartForCodeBlock(
	context: ConversionContext,
	block: CodeBlockBlock
): number | undefined {
	// Get the code content from the block
	const [, code] = block.c as [[string, string[], [string, string][]], string];

	// Search for this code in the source to find where it starts
	// The fence will be on the line before
	const codeBytes = new TextEncoder().encode(code);
	const sourceBytes = context.sourceBytes;

	// Find the code in the source
	const codeStart = findBytesInSource(sourceBytes, codeBytes);
	if (codeStart === -1) {
		return undefined;
	}

	// Go back to find the start of the fence line (line before the code)
	let pos = codeStart;

	// First, go back to find the start of the current line
	while (pos > 0 && sourceBytes[pos - 1] !== 0x0A) {
		pos--;
	}

	// Now go back one more line to find where the fence line starts
	if (pos > 0) {
...
/**
 * Find the start of the fence line before a raw block.
 */
function findFenceStartForRawBlock(
	context: ConversionContext,
	block: RawBlock
): number | undefined {
	// RawBlock content: [format, string]
	...
	const contentStart = findBytesInSource(sourceBytes, contentBytes);
	if (contentStart === -1) {
		return undefined;
	}

	// Go back to find the start of the fence line
...
/**
 * Find a byte sequence in the source bytes.
 */
function findBytesInSource(source: Uint8Array, target: Uint8Array): number {
