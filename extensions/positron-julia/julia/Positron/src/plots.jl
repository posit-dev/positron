...
mutable struct PlotsService
    ...
    function PlotsService()
        new(
            ...
            Dict{String,Any}(),
            nothing,
            Dict{String,Any}("width" => 800, "height" => 600, "pixel_ratio" => 1.0),
        )
    end
end
...
function handle_render(service::PlotsService, request::PlotRenderParams)
    ...
    height =
        request.size !== nothing ? request.size.height : service.render_settings["height"]
    ...
    try
        ...
        settings =
            PlotRenderSettings(PlotSize(height, width), request.pixel_ratio, request.format)
