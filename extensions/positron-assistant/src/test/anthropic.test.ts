...
suite('AnthropicLanguageModel', () => {
	...
	suite('provideLanguageModelResponse response streaming behaviour', () => {
		test('token usage is streamed back as part of the response', async () => {
			...
			const initialExpected = { type: 'usage', data: { input_tokens: 7200, output_tokens: 0 } };
			...
			assert.deepStrictEqual(JSON.parse(decoder.decode(initialData.data)), initialExpected, 'Initial usage data should decode as expected');

			const finalData = progress.report.getCall(1).args[0].part;
			const finalExpected = { type: 'usage', data: { input_tokens: 7200, output_tokens: 2000 } };
			...
			assert.deepStrictEqual(JSON.parse(decoder.decode(finalData.data)), finalExpected, 'Final usage data should decode as expected');
		});
	});
