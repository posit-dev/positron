...
import { NotebookCellEditProposal, NotebookCellEditType, NotebookCellEditOperation, PositronAssistantToolName } from '../types.js';
...
// --- Notebook Edit Proposal Handling ---

/**
 * Parses a notebook edit proposal from a tool result.
 * Returns the proposal if it's a valid notebook cell edit, null otherwise.
 */
function parseNotebookEditProposal(result: vscode.LanguageModelToolResult): NotebookCellEditProposal | null {
 ...
 */
async function applyNotebookEditProposal(
...
/**
 * Handles notebook tool results that contain edit proposals.
 * If the result is a notebook cell edit proposal, applies it via response.textEdit()
 * and returns a result message. Otherwise, returns undefined to indicate the result
 * should be passed through unchanged.
 *
 * @param toolName The name of the tool that was invoked
 * @param result The tool result to check
 * @param response The chat response stream for emitting edits
 * @returns The result to use if handled, or undefined if not handled
 */
export async function handleNotebookEditProposal(
	toolName: string,
	result: vscode.LanguageModelToolResult,
	response: vscode.ChatResponseStream
): Promise<vscode.LanguageModelToolResult | undefined> {
	// Only handle EditNotebookCells tool
	if (toolName !== PositronAssistantToolName.EditNotebookCells) {
		return undefined;
	}

	const proposal = parseNotebookEditProposal(result);
	if (!proposal) {
		return undefined;
	}

	try {
		await applyNotebookEditProposal(proposal, response);
		// Return a success message instead of the raw proposal
		return new vscode.LanguageModelToolResult([
			new vscode.LanguageModelTextPart(`Successfully proposed edit to cell ${proposal.cellIndex}`)
		]);
	} catch (error) {
		...
		return new vscode.LanguageModelToolResult([
			new vscode.LanguageModelTextPart(`Error applying notebook cell edit to cell ${proposal.cellIndex}`),
