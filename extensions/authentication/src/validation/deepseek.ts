...
export async function validateDeepseekApiKey(apiKey: string, config: positron.ai.LanguageModelConfig): Promise<void> {
	...
	try {
		...
		if (response.status === 401 || response.status === 403) {
			throw new ApiKeyValidationError(vscode.l10n.t('Invalid DeepSeek API key'));
		}
		...
		if (err instanceof Error && err.name === 'AbortError') {
			throw new ApiKeyValidationError(vscode.l10n.t('Could not validate DeepSeek API key within {0} seconds', String(KEY_VALIDATION_TIMEOUT_MS / 1000)));
		}
		if (err instanceof ApiKeyValidationError) {
			throw err;
		}
		throw new ApiKeyValidationError(vscode.l10n.t('Could not validate DeepSeek API key. Check your network connection and try again.'));
	} finally {
