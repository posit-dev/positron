...
import { pathExists, readFile, resolveSymbolicLink } from '../externalDependencies';
 ...
 * or be located in (or symlinked to) ~/.local/share/uv.
...
export async function isUvEnvironment(interpreterPath: string): Promise<boolean> {
    ...
    if (normalizedInterpreterPath.startsWith(path.normalize(uvDir))) {
        return true;
    }
    ...
    try {
        ...
        if (resolvedPath !== interpreterPath && resolvedPath.startsWith(path.normalize(uvDir))) {
            return true;
        }
    } catch (ex) {
        // If we can't resolve the symlink, continue with other checks
    }

    // Check for pyvenv.cfg with uv marker
