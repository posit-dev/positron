...
export class PythonRuntimeManager implements IPythonRuntimeManager, Disposable {
     ...
     *
     * - Added (`new` only): register a runtime for the new path.
    ...
    private async handleInterpreterChange(event: PythonEnvironmentsChangedEvent): Promise<void> {
        if (!event.old && event.new) {
            await this.registerLanguageRuntimeFromPath(event.new.path);
    ...
    async registerLanguageRuntimeFromPath(
        ...
        if (interpreter) {
            ...
            // If this path is already registered as the same (resolved) runtime,
            // there's nothing to do. If it resolved to a different runtime (e.g. the
            // real version supersedes a stale one from discovery), registerLanguageRuntime
            // retracts the stale entry so the picker shows the correct version.
            if (
