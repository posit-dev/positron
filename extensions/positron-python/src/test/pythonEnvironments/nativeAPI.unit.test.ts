...
// --- Start Positron ---
suite('mergeNativeAndModuleEnvs', () => {
    function makeEnv(filename: string, kind: PythonEnvKind, name = ''): PythonEnvInfo {
        return {
            arch: Architecture.Unknown,
            id: filename,
    ...
    test('prefers the module env when native discovery found the same path', () => {
        // Regression: a module-managed interpreter that is also on PATH (because
        // the user `module load`ed it before launching Positron) was discovered
        // natively as a plain System interpreter, shadowing the Module env and
        // dropping the `module load ...` startup metadata.
        const sharedPath = '/opt/software/python/3.12.8/bin/python3';
        const nativeEnvs = [makeEnv(sharedPath, PythonEnvKind.System)];
        const moduleEnvs = [makeEnv(sharedPath, PythonEnvKind.Module, 'Python-Leaves')];

        const merged = nativeAPI.mergeNativeAndModuleEnvs(nativeEnvs, moduleEnvs);

        assert.deepEqual(merged, moduleEnvs);
    });

    test('keeps distinct native and module envs', () => {
        const nativeEnvs = [makeEnv('/usr/bin/python3', PythonEnvKind.System)];
        const moduleEnvs = [makeEnv('/opt/software/python/3.12.8/bin/python3', PythonEnvKind.Module, 'Python-Leaves')];

        const merged = nativeAPI.mergeNativeAndModuleEnvs(nativeEnvs, moduleEnvs);

        assert.deepEqual(merged, [...nativeEnvs, ...moduleEnvs]);
    });
});
// --- End Positron ---
