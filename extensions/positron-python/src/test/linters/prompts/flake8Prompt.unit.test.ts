...
import { Flake8ExtensionPrompt, FLAKE8_EXTENSION } from '../../../client/linters/prompts/flake8Prompt';
...
suite('Flake8 Extension prompt tests', () => {
    ...
    test('User selected: install extension (insiders)', async () => {
        ...
        showInformationMessageStub.resolves(ToolsExtensions.installFlake8Extension);
        assert.isTrue(await prompt.showPrompt());

        executeCommandStub.calledOnceWith('workbench.extensions.installExtension', FLAKE8_EXTENSION, {
