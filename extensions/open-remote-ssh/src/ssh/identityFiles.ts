...
export interface SSHKey {
	...
	isPrivate?: boolean;
	certificate?: Buffer; // Certificate data for -cert.pub files
}
...
export async function gatherIdentityFiles(identityFiles: string[], sshAgentSock: string | undefined, identitiesOnly: boolean, logger: Log) {
	...
	const identityFileContentsResult = await Promise.allSettled(identityFiles.map(async keyPath => {
		// Check for certificate file first (e.g., id_rsa-cert.pub)
		const certPath = keyPath + '-cert.pub';
		const hasCert = await fileExists(certPath);

		// Then check for public key
		const pubPath = keyPath + '.pub';
		const hasPub = await fileExists(pubPath);

		// Determine which file to read for the public key/cert
		const readPath = hasCert ? certPath : (hasPub ? pubPath : keyPath);
		const keyData = await fs.promises.readFile(readPath);
	...
	const fileKeys: SSHKey[] = identityFileContentsResult.map((result, i) => {
		...
		const { keyData, hasCertificate, keyPath } = result.value;
		const parsedResult = ssh2.utils.parseKey(keyData);
		if (parsedResult instanceof Error || !parsedResult) {
			logger.error(`Error while parsing SSH public key ${keyPath}:`, parsedResult);
			return undefined;
		}
		...
		return {
			...
			parsedKey,
			fingerprint,
			certificate: hasCertificate ? keyData : undefined
		};
