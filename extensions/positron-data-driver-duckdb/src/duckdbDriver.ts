 ...
 * the database as the user wrote it, and any embedded `"""` is escaped so it cannot close the literal
 * early.
 *
 ...
 * as a fourth quote. The whitespace they add around the SQL is insignificant to the database.
 */
 ...
 * variants created. Exported (and called by the driver's `generateQueryCode`) so it can be tested
 * without an extension context.
 */
export function generateQueryCode(request: positron.QueryCodeRequest): string | undefined {
	switch (request.languageId) {
		case 'python':
			switch (request.variantId) {
				case 'duckdb':
					// DuckDB's own relational API returns a data frame from .df(), so this variant needs no
					// pandas import of its own.
					return `${request.connectionVariable}.sql(${pythonStringLiteral(request.query)}).df()`;
				case 'sqlalchemy':
					// pandas reads from a SQLAlchemy connectable, taking the query as a plain string, and
					// returns a data frame worth printing.
