...
description: >
  ...
  "set up a Python environment", "run my shiny app"{{#if pwb}}, "my app URL
  doesn't load"{{/if}}.
---
...
`executeCode` or a raw terminal command. The commands it documents manage the
terminal, detect the app URL, set up any proxying the environment
requires{{#if pwb}} (required on Posit Workbench, where a raw localhost URL is
unreachable from the user's browser){{/if}}, and preview the app -- in the
