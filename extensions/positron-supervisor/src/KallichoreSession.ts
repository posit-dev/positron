...
export class KallichoreSession implements JupyterLanguageRuntimeSession {
	...
	async createServerComm(target_name: string, host: string): Promise<[RawComm, number]> {
		this.log(`Starting server comm ${target_name} for ${host}`, vscode.LogLevel.Debug);
		...
		this.log(`Started server comm ${target_name} for ${host} on port ${port}`, vscode.LogLevel.Debug);
		return [comm, port];
	}

	/**
	 * Creates a DAP communication channel and returns both the comm and the port.
	 *
	 * @param target_name The name of the comm target
	 ...
	 * @returns A promise that resolves to a tuple of [RawComm, port number]
	 */
	async createDapComm(target_name: string, debugType: string, debugName: string): Promise<[RawComm, number]> {
		...
		return this.createServerComm(target_name, ipAddress);
	}
