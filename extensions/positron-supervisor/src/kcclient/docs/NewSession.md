...
**workspace_id** | **string** | The MCP workspace creating the session, if the client has registered one. The session belongs to that workspace: agents reach it through that workspace\&#39;s MCP endpoint and no other. | [optional] [default to undefined]
