...
// `~`/`~*` regex operators, boolean literals, `ctid` tiebreaker, Date coercion, FLOOR cast).
...
/** A column in a PostgreSQL table or view, with its declared type and resolved display type. */
export interface PostgresSchemaEntry {
	column_name: string;
	/** The PostgreSQL type from information_schema (e.g. 'integer', 'character varying'). */
	column_type: string;
	type_display: ColumnDisplayType;
}

/**
 * Maps a PostgreSQL column type name (from information_schema.columns.data_type) to a Data Explorer
 * display type.
 */
export function postgresDisplayType(dataType: string): ColumnDisplayType {
	...
	// uuid, json, ARRAY, USER-DEFINED (enums), etc. render as strings.
	return ColumnDisplayType.String;
}

/** Quotes and escapes an identifier for PostgreSQL by doubling embedded double-quotes. */
...
/**
 * Builds a SQL WHERE expression for a single row filter, in PostgreSQL dialect: set membership uses
 * `IN (...)`, booleans compare to `true`/`false`, and regex uses the `~` / `~*` operators.
 */
...
/**
 * Serves Data Explorer requests for a single PostgreSQL table or view. Translates each protocol
 ...
 */
export class PostgresTableView {
	 ...
	 * @param objectKind Whether this is a table (has a stable ctid) or a view.
	 * @param schema The resolved column schema.
	 */
	constructor(
		private readonly client: IPostgresQueryClient,
	...
	/**
	 * Formats a raw PostgreSQL value into the Data Explorer cell encoding: a sentinel number for
		...
		switch (displayType) {
			...
			case ColumnDisplayType.Object:
				if (value instanceof Uint8Array) {
					return `[bytea ${value.byteLength} bytes]`;
				}
	...
	/**
	 * Builds an ORDER BY clause for the given sort keys. For tables a trailing `ctid` is appended as
	 * a stable tiebreaker so pagination is deterministic; views have no ctid, so they omit it.
	 */
	...
	async exportDataSelection(params: ExportDataSelectionParams): Promise<ExportedData> {
		...
		const runExport = async (query: string, columns: Array<PostgresSchemaEntry>): Promise<ExportedData> => {
			const rows = await this.client.runQuery(query);
			const matrix = [
		...
		const selectorsFor = (columns: Array<PostgresSchemaEntry>) =>
	 ...
	 * ctid, mirroring the requested-row ordering.
	 */
	...
	private async _summaryStats(
		...
		if (display === ColumnDisplayType.Boolean) {
			// PostgreSQL has real booleans, so test the column directly rather than comparing to 0/1.
			const rows = await this.client.runQuery(
	...
	private _emptySummaryStats(entry: PostgresSchemaEntry): ColumnSummaryStats {
