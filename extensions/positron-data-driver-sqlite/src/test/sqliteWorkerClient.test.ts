...
suite('SQLite worker isolation', () => {
	// A stub worker that speaks the IPC protocol, so we can exercise crash
	// recovery without depending on the native binding or a real corrupt file. It
	// crashes hard on a sentinel query and otherwise returns a trivial one-row
	// result.
	const STUB_WORKER = `
	...
	test('survives a worker crash, rejects the in-flight query, and respawns', async () => {
		const dir = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'sqlite-stub-worker-'));
		const stubPath = path.join(dir, 'stubWorker.js');
		await fs.promises.writeFile(stubPath, STUB_WORKER);

		const client = new SqliteWorkerClient({ databasePath: ':memory:', readOnly: false }, stubPath);
		let crashCount = 0;
