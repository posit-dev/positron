...
/** A column in a SQLite table or view, with its declared type and resolved display type. */
export interface SqliteSchemaEntry {
	column_name: string;
	/** The declared SQLite type (e.g. 'INTEGER', 'VARCHAR(50)'), or '' if untyped. */
	column_type: string;
	type_display: ColumnDisplayType;
}

/**
 * Maps a SQLite declared column type to a Data Explorer display type. SQLite has dynamic typing
 * with only "type affinity" rules, so we first recognize the common date/time/boolean type names
 * (which carry NUMERIC affinity but display specially), then fall back to the affinity rules from
 * the SQLite documentation.
 */
export function sqliteDisplayType(declaredType: string): ColumnDisplayType {
	const type = declaredType.toUpperCase();

	// Names that have NUMERIC affinity in SQLite but display as a more specific type.
	...
	// SQLite affinity rules, applied in order.
	...
	// NUMERIC and anything else: treat as floating-point for display purposes.
	return ColumnDisplayType.Floating;
}
...
/**
 * Builds a SQL WHERE expression for a single row filter. The SQLite dialect differs from DuckDB:
 * set membership uses `IN (...)` (not `IN [...]`) and regex uses the `regexp`/`regexpi` functions
		...
		case RowFilterType.Search: {
			...
			switch (params.search_type) {
				...
				case TextSearchType.RegexMatch: {
					const fn = params.case_sensitive ? 'regexp' : 'regexpi';
					return `${fn}('${quoteLiteral(params.term)}', ${quotedName})`;
				}
			}
			return '1=1';
		}
...
/**
 * Serves Data Explorer requests for a single SQLite table or view. Translates each protocol method
 * into SQL run through the connection's worker client. Values are fetched raw and formatted in
 * TypeScript (SQLite lacks DuckDB's FORMAT/isinf/isnan), while filtering, sorting, counts, and
 * aggregations are pushed into SQL.
 */
export class SqliteTableView {
	...
	/**
	 * Formats a raw SQLite value into the Data Explorer cell encoding: a sentinel number for
	...
	private _formatValue(value: unknown, displayType: ColumnDisplayType, opts: FormatOptions): ColumnValue {
		...
		switch (displayType) {
			...
			case ColumnDisplayType.Boolean:
				return typeof value === 'number' ? (value ? 'true' : 'false') : truncate(String(value), opts);
	...
	async exportDataSelection(params: ExportDataSelectionParams): Promise<ExportedData> {
		...
		const selectorsFor = (columns: Array<SqliteSchemaEntry>) =>
	...
	private _emptySummaryStats(entry: SqliteSchemaEntry): ColumnSummaryStats {
