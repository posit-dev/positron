...
suite('SuggestModel - offWhenInlineCompletions with InlineCompletionsController', function () {
	...
	test('does not trigger after the inline model is disposed mid-wait (e.g., readonly toggled)', async function () {
		...
		await withSuggestModelAndInlineCompletions('abc def', inlineProvider, async (suggestModel, editor) => {
			...
			// Let _waitForInlineCompletionsAndTrigger be scheduled and set up. This must
			// exceed Positron's quickSuggestionsDelay (250ms, vs upstream's 10ms) so the
			// wait machinery is actually established while the inline model still exists;
			// otherwise the trigger body runs only after readonly is toggled and takes the
			// early-out path instead of exercising the dispose-mid-wait guard under test.
			await timeout(300);
