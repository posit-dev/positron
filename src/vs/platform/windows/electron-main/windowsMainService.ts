...
export class WindowsMainService extends Disposable implements IWindowsMainService {
	...
	private async openInBrowserWindow(options: IOpenBrowserWindowOptions): Promise<ICodeWindow> {
		...
		const configuration: INativeWindowConfiguration = {
			...
			// --- Start Positron ---
			// Canvas is one product surface per application instance: a window
			// opened while another window presents Canvas must not enter Canvas
			// itself. For a reused window, "elsewhere" is relative to that
			// window, so the one presenting Canvas is not locked out of its own
			// re-entry. Refreshed on reload (CodeWindow#reload).
			canvasModeEngagedElsewhere: window
				? this.positronCanvasModeMainService.isEngagedElsewhere(window.id)
				: this.positronCanvasModeMainService.isEngaged,
			// --- End Positron ---
