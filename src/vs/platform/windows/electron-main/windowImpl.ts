...
export class CodeWindow extends BaseWindow implements ICodeWindow {
	...
	async reload(cli?: NativeParsedArgs): Promise<void> {
		...
		// restored through workspace storage, so a later explicit exit stays exited.
		delete configuration.canvas;

		// The engagement stamp describes open time; a reload happens later, so
		// refresh it. Without this, a window opened while Canvas was engaged
		// elsewhere would stay locked out of Canvas mode after that engagement
		// ended, for as long as it only ever reloaded.
		configuration.canvasModeEngagedElsewhere = this.positronCanvasModeMainService.isEngagedElsewhere(this.id);
		// --- End Positron ---
