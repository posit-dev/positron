...
export class EditorGroupView extends Themable implements IEditorGroupView {
	...
	layout(width: number, height: number, top: number, left: number): void {
		...
		// height is subtracted here and added to the editor top alongside the header row.
		const editorHeight = Math.max(0, height - editorActionBarHeight - titleControlSize.height - headerBoxHeight);
		this.editorContainer.style.width = `${contentWidth}px`;
		this.editorContainer.style.height = `${editorHeight}px`;
		this.editorPane.layout({ width: contentWidth, height: editorHeight, top: top + editorActionBarHeight + titleControlSize.height + headerBoxHeight, left });
		// --- End Positron ---
	}
