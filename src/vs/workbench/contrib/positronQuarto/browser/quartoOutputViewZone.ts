...
export class QuartoOutputViewZone extends Disposable implements IViewZone {
	...
	private async _renderHtmlInWebview(content: string, output: ICellOutput, container: HTMLElement): Promise<void> {
		...
		try {
			...
			// Render raw HTML content as a self-contained document (i.e. R
			// leaflet map). Using `createNotebookOutputWebview()` would find
			// the built in renderer for `text/html` and flatten the
			// self-contained document, dropping `<head>` and scripts.
