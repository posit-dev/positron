...
/**
 * Translation helpers between a `.qmd` document and the shadow notebook cells
 * that mirror its code chunks.
 *
 * A cell's text model contains exactly the code lines of the chunk (the lines
 * from `codeStartLine` to `codeEndLine`, joined). Code chunk lines are copied
 * verbatim, so only the line number is shifted between the two coordinate
 * spaces; columns are identical.
 *
 * The column identity relies on chunks starting at column 0. That is
 * guaranteed by the parser: `CODE_START_REGEX` in `quartoParser.ts` anchors
 * the opening fence at line start (`/^```\{...`), so an indented fence never
 * produces a cell at all (covered by a test in
 * `quartoPositionMapping.vitest.ts`). If the parser ever learns to recognize
 * indented chunks, these helpers must gain a column offset.
 *
