...
/**
 * Resolve the targeted image data URL and owning cell index for a plot action.
 * Prefers a forwarded `CopyImageMenuArg` (right-click on a specific image),
 * otherwise falls back to the first image output of the active cell.
 */
function resolveImageOutputTarget(notebook: IPositronNotebookInstance, args: unknown[]): { dataUrl: string; cellIndex: number } | undefined {
	const state = notebook.selectionStateMachine.state.get();
	const cell = getActiveCell(state);
	if (!cell?.isCodeCell()) {
		return undefined;
...
// Save a plot image output to disk (menu-driven)
...
// Open a plot image output in a new editor tab (menu-driven)
