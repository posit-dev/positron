...
export class CellContextKeyManager extends Disposable {
	readonly isCode: IContextKey<boolean>;
	readonly isMarkdown: IContextKey<boolean>;
	readonly isRaw: IContextKey<boolean>;
	readonly isRunning: IContextKey<boolean>;
	readonly isPending: IContextKey<boolean>;
	readonly isFirst: IContextKey<boolean>;
	readonly isLast: IContextKey<boolean>;
	readonly isOnly: IContextKey<boolean>;
	readonly markdownEditorOpen: IContextKey<boolean>;
	readonly isSelected: IContextKey<boolean>;
	readonly isActive: IContextKey<boolean>;
	readonly canMoveUp: IContextKey<boolean>;
	readonly canMoveDown: IContextKey<boolean>;
	readonly hasOutputs: IContextKey<boolean>;
	readonly imageOutputCount: IContextKey<number>;
	readonly jsonOutputCount: IContextKey<number>;
	readonly outputIsCollapsed: IContextKey<boolean>;
	readonly outputScrolling: IContextKey<boolean>;
	readonly outputOverflows: IContextKey<boolean>;
	...
	constructor(
		...
		const globalOutputScrollingObs = observableFromEvent(
			Event.filter(instance.notebookOptions.onDidChangeOptions, e => Boolean(e.outputScrolling), this._store),
			() => instance.notebookOptions.getLayoutConfiguration().outputScrolling
		);
