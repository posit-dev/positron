...
describe('PositronNotebookInstance', () => {
	...
	describe('Test Harness', () => {
		it('notebook has cells from notebook text model', () => {
			...
			expect(cells[0].model.getValue()).toBe('print("hello")', 'Unexpected content for notebook cell 0');
			expect(cells[1].model.getValue()).toBe('print("world")', 'Unexpected content for notebook cell 1');

			const { textModel } = notebook;
			expect(textModel, 'Notebook should have a text model').toBeTruthy();
			expect(textModel.cells[0].getValue()).toBe('print("hello")', 'Unexpected content for text model cell 0');
			expect(textModel.cells[1].getValue()).toBe('print("world")', 'Unexpected content for text model cell 1');
		});
	});
