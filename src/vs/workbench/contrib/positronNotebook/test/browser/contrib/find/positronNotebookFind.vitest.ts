...
describe('PositronNotebookFindController', () => {
	// positronNotebookInstantiationService creates its own instantiation service internally;
	...
	describe('Matching Logic', () => {
		...
		it('match ordering is stable: within-cell positions preserved', () => {
			...
			expect(matches[0].cellRange.isBefore(matches[1].cellRange)).toBeTruthy();
		});
	...
	describe('Focus and Visibility', () => {
		...
		it('context key FIND_WIDGET_VISIBLE toggles correctly', () => {
			...
			expect(notebook.scopedContextKeyService.getContextKeyValue(CONTEXT_FIND_WIDGET_VISIBLE.key)).toBe(true);

			controller.hide();
			expect(notebook.scopedContextKeyService.getContextKeyValue(CONTEXT_FIND_WIDGET_VISIBLE.key)).toBe(false);
		});
		...
		it('context key FIND_INPUT_FOCUSED is true immediately after show', () => {
			...
			expect(notebook.scopedContextKeyService.getContextKeyValue(CONTEXT_FIND_INPUT_FOCUSED.key)).toBe(true);
		});
		...
		it('context key FIND_INPUT_FOCUSED is false after hide', () => {
			...
			controller.hide();
			expect(notebook.scopedContextKeyService.getContextKeyValue(CONTEXT_FIND_INPUT_FOCUSED.key)).toBe(false);
		});
		...
		it('context key REPLACE_INPUT_FOCUSED is false after show and false after hide', () => {
			...
			expect(notebook.scopedContextKeyService.getContextKeyValue(CONTEXT_REPLACE_INPUT_FOCUSED.key)).toBe(false);
	...
	describe('Performance Optimizations', () => {
		...
		it('replaceAll() replaces all matches beyond the decoration limit', () => runWithFakedTimers({}, async () => {
			...
			expect(!cell.model.textModel!.getValue().includes('aa')).toBeTruthy();
