...
describe('Positron - Plots Service', () => {
	...
	it('history policy: change event', async () => {
		...
		await raceTimeout(didChangeHistoryPolicy, 100, () => (() => { throw new Error('onDidChangeHistoryPolicy event did not fire'); })());
	...
	it('display location: change event', async () => {
		...
		await raceTimeout(didChangeDisplayLocation, 100, () => (() => { throw new Error('onDidChangeDisplayLocation event did not fire'); })());
	...
	it('sizing policy: change event', async () => {
		...
		await raceTimeout(didClosePlot, 100, () => (() => { throw new Error('onDidChangeSizingPolicy event did not fire'); })());

		expect(sizingPolicyChanged, 'onDidChangeSizingPolicy event should fire').toBeTruthy();
	...
	it('selection: select plot', async () => {
		...
		await raceTimeout(didSelectPlot, 100, () => (() => { throw new Error('onDidSelectPlot event did not fire'); })());

		expect(selectPlotCalled, 'onDidSelectPlot event should fire').toBeTruthy();
	...
	it('selection: remove selected plot', async () => {
		...
		await raceTimeout(didRemovePlot, 100, () => (() => { throw new Error('onDidRemovePlot event did not fire'); })());

		expect(removePlotCalled, 'onDidRemovePlot event should fire').toBeTruthy();
