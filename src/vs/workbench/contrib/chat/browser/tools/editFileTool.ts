...
export class EditTool implements IToolImpl {
	...
	async invoke(invocation: IToolInvocation, countTokens: CountTokensCallback, progress: ToolProgress, token: CancellationToken): Promise<IToolResult> {
		...
		// Listen for user actions (accept/reject) on this file
		...
		if (editResult === 'rejected') {
			...
			const chatWidget = this.chatWidgetService.getWidgetBySessionId(model.sessionId);
			if (chatWidget) {
				chatWidget.setInputPlaceholder('Provide feedback on the proposed edit...');
			}

			// Listen for the user's feedback submission, then reset placeholder
			const feedbackDisposable = this.chatService.onDidSubmitRequest((event) => {
				if (event.chatSessionId === model.sessionId) {
			...
			// Return the tool result first so it gets saved to conversation history
			const result: IToolResult = {
				content: [{ kind: 'text' as const, value: 'The user rejected the edit and might provide feedback.' }]
			};

			// Cancel the request so that the user can provide feedback, but only after the tool result is appended to the history
			setTimeout(() => {
				this.chatService.cancelCurrentRequestForSession(model.sessionId);
			}, 0);
