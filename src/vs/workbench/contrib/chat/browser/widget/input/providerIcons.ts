...
// Copilot and OpenAI use parameterized fill colors because their SVGs use
// fill="currentColor" which doesn't resolve when embedded as data URI background
// images. We pass an explicit fill color based on the active theme.
function copilotSvg(fill: string): string {
	return `<svg width="24" height="24" viewBox="0 0 24 24" fill="${fill}" xmlns="http://www.w3.org/2000/svg">
...
function openaiSvg(fill: string): string {
	return `<svg width="24" height="24" viewBox="0 0 24 24" fill="${fill}" fill-rule="evenodd" xmlns="http://www.w3.org/2000/svg">
...
function positAiSvg(fill: string): string {
	return `<?xml version="1.0" encoding="UTF-8"?><svg id="Layer_2" xmlns="http://www.w3.org/2000/svg" viewBox="38.5 -2.7 80 80" x="0px" y="0px"><path fill="${fill}" d="M77.37,60.07c.66.66,1.73.66,2.39,0l30.06-30.06,5.81,5.81c.66.66.66,1.73,0,2.39l-35.87,35.87c-.66.66-1.73.66-2.39,0l-35.87-35.87c-.66-.66-.66-1.73,0-2.39l5.81-5.81,30.06,30.06Z"/><path fill="${fill}" d="M79.71,14.56c-.66-.66-1.73-.66-2.39,0l-16.33,16.33-5.81-5.81c-.66-.66-.66-1.73,0-2.39L77.32.56c.66-.66,1.73-.66,2.39,0l22.22,22.22c.66.66.66,1.73,0,2.39l-5.81,5.81-16.41-16.41Z"/><rect fill="${fill}" x="70.86" y="28.95" width="15.4" height="15.4" rx="1.82" ry="1.82" transform="translate(-2.9 66.28) rotate(-45)"/></svg>`;
}
 ...
 * @param isDarkTheme Whether the current theme is dark. Used to select the
 * appropriate fill color for monochrome icons (Copilot, OpenAI, Posit AI) that
 * are rendered as data URI background images where CSS currentColor is unavailable.
 */
export function getProviderIcon(vendor: string, isDarkTheme?: boolean): ProviderIcon | undefined {
	const monoFill = isDarkTheme ? '#FFFFFF' : '#000000';
	switch (vendor) {
		case 'copilot':
			return createSvgIcon(copilotSvg(monoFill));
		case 'anthropic':
		case 'anthropic-api':
			return createSvgIcon(claudeSvg);
		case 'openai':
		case 'openai-api':
			return createSvgIcon(openaiSvg(monoFill));
		case 'google':
		...
		case 'posit-ai':
			return createSvgIcon(positAiSvg(monoFill));
