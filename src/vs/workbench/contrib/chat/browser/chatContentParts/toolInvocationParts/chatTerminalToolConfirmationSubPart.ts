...
export class ChatTerminalToolConfirmationSubPart extends BaseChatToolInvocationSubPart {
	...
	constructor(
		...
		// TODO 1.104.0 Is this used for approving code to run?
		// --- End Positron ---
		...
		*/
		...
		const model = this._register(this.modelService.createModel(terminalData.commandLine.toolEdited ?? terminalData.commandLine.original,
			this.languageService.createById(languageId),
			resource,
			true
		));
