	...
	private renderWelcomeViewContentIfNeeded() {
		...
		if (!numItems) {
			...
			*/
			...
			if (this.contextKeyService.contextMatchesRules(this.chatSetupTriggerContext) && useExpView) {
...
Type \`/\` to use predefined commands such as \`/help\`.`,
	...
	layout(height: number, width: number): void {
		...
		// --- Start Positron ---
		const contentHeight = Math.max(0, height - inputHeight - chatTodoListWidgetHeight - actionBarHeight);
		// --- End Positron ---
		if (this.viewOptions.renderStyle === 'compact' || this.viewOptions.renderStyle === 'minimal') {
