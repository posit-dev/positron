...
describe('ConnectProviderView', () => {
	...
	it('renders a Connect footer button and legal text for Posit AI', () => {
		rtl.render(<ConnectProviderView source={positAi} onAction={async () => { }} onBack={vi.fn()} onClose={vi.fn()} onEditRawConfig={vi.fn()} />);
	...
	it('dispatches oauth-signin when Connect is clicked', async () => {
		...
		rtl.render(<ConnectProviderView source={positAi} onAction={onAction} onBack={vi.fn()} onClose={vi.fn()} onEditRawConfig={vi.fn()} />);
	...
	it('invokes onBack and onClose from the footer buttons', async () => {
		...
		const onClose = vi.fn();
		const user = userEvent.setup();
		rtl.render(<ConnectProviderView source={positAi} onAction={async () => { }} onBack={onBack} onClose={onClose} onEditRawConfig={vi.fn()} />);
	...
	it('reports a cancel handler while an OAuth sign-in is in progress that dispatches cancel', async () => {
		...
		const user = userEvent.setup();
		rtl.render(<ConnectProviderView source={positAi} onAction={onAction} onBack={vi.fn()} onClose={vi.fn()} onEditRawConfig={vi.fn()} onPendingSignInChange={cancel => { reportedCancel = cancel; }} />);
	...
	it('renders an API key input for an API-key provider', () => {
		rtl.render(<ConnectProviderView source={anthropic} onAction={async () => { }} onBack={vi.fn()} onClose={vi.fn()} onEditRawConfig={vi.fn()} />);
	...
	it('collects an API key and dispatches save when Connect is clicked', async () => {
		const onAction = vi.fn().mockResolvedValue(undefined);
		const user = userEvent.setup();
		rtl.render(<ConnectProviderView source={anthropic} onAction={onAction} onBack={vi.fn()} onClose={vi.fn()} onEditRawConfig={vi.fn()} />);
		await user.type(screen.getByLabelText(/api key/i), 'sk-test');
		await user.click(screen.getByRole('button', { name: 'Connect' }));
	...
	it('shows the base URL input and no API key for a base-URL-only provider', () => {
		rtl.render(<ConnectProviderView source={lmstudio} onAction={async () => { }} onBack={vi.fn()} onClose={vi.fn()} onEditRawConfig={vi.fn()} />);
	...
	it('labels the Databricks base URL input as the workspace URL', () => {
		rtl.render(<ConnectProviderView source={databricks} onAction={async () => { }} onBack={vi.fn()} onClose={vi.fn()} onEditRawConfig={vi.fn()} />);
	...
	it('labels the Snowflake base URL input as the account identifier', () => {
		rtl.render(<ConnectProviderView source={snowflake} onAction={async () => { }} onBack={vi.fn()} onClose={vi.fn()} onEditRawConfig={vi.fn()} />);
	...
	it('dispatches save with the base URL for a base-URL-only provider', async () => {
		const onAction = vi.fn().mockResolvedValue(undefined);
		const user = userEvent.setup();
		rtl.render(<ConnectProviderView source={lmstudio} onAction={onAction} onBack={vi.fn()} onClose={vi.fn()} onEditRawConfig={vi.fn()} />);
		const baseUrlInput = screen.getByLabelText(/base url/i);
		await user.clear(baseUrlInput);
	...
	it('does not render the API Type selector while it is deferred (#13817)', () => {
		rtl.render(<ConnectProviderView source={custom} onAction={async () => { }} onBack={vi.fn()} onClose={vi.fn()} onEditRawConfig={vi.fn()} />);
	...
	it('dispatches OpenAI Chat Completions as the API type', async () => {
		const onAction = vi.fn().mockResolvedValue(undefined);
		const user = userEvent.setup();
		rtl.render(<ConnectProviderView source={custom} onAction={onAction} onBack={vi.fn()} onClose={vi.fn()} onEditRawConfig={vi.fn()} />);
		await user.type(screen.getByLabelText(/api key/i), 'sk-test');
		await user.click(screen.getByRole('button', { name: 'Connect' }));
	...
	it('spins only the Disconnect button (not Connect) while an error-state disconnect is in flight', async () => {
		...
		const user = userEvent.setup();
		rtl.render(<ConnectProviderView source={erroredAnthropic} onAction={onAction} onBack={vi.fn()} onClose={vi.fn()} onEditRawConfig={vi.fn()} />);
	...
	it('keeps Connect disabled while disconnecting an errored, signed-in OAuth provider', async () => {
		...
		const onAction = vi.fn().mockImplementation(() => new Promise<void>(resolve => { resolveDisconnect = resolve; }));
		const user = userEvent.setup();
		rtl.render(<ConnectProviderView source={erroredPositAi} onAction={onAction} onBack={vi.fn()} onClose={vi.fn()} onEditRawConfig={vi.fn()} />);
		await user.click(screen.getByRole('button', { name: 'Disconnect' }));
		expect(screen.getByRole('button', { name: 'Connect' })).toBeDisabled();
		await act(async () => { resolveDisconnect(); });
	});
	...
	describe('with multiple auth methods (Databricks)', () => {
		it('names the auth method radio group', () => {
			rtl.render(<ConnectProviderView source={databricksOAuth} onAction={async () => { }} onBack={vi.fn()} onClose={vi.fn()} onEditRawConfig={vi.fn()} />);
		...
		it('dispatches oauth-signin by default', async () => {
			const onAction = vi.fn().mockResolvedValue(undefined);
			const user = userEvent.setup();
			rtl.render(<ConnectProviderView source={databricksOAuth} onAction={onAction} onBack={vi.fn()} onClose={vi.fn()} onEditRawConfig={vi.fn()} />);
			await user.click(screen.getByRole('button', { name: 'Connect' }));
		...
		it('hides the picker while signed in', () => {
			rtl.render(<ConnectProviderView source={{ ...databricksOAuth, signedIn: true }} onAction={async () => { }} onBack={vi.fn()} onClose={vi.fn()} onEditRawConfig={vi.fn()} />);
