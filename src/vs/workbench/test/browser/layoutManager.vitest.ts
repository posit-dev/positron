...
describe('LayoutManager', () => {
	...
	const verifySizeOfDefaultSizedEntries = (defaultSize: number, entries: number) => {
		...
		// Verify size.
		expect(layoutManager.unpinnedLayoutEntriesSize).toBe(size);
		...
		// Verify size.
		expect(layoutManager.unpinnedLayoutEntriesSize).toBe(size + defaultSize);
	...
	const verifySizeOfFixedSizedEntries = (entrySize: number, entries: number) => {
		...
		// Verify size.
		expect(layoutManager.unpinnedLayoutEntriesSize).toBe(size + entrySize);
	...
	const verifySizeOfRandomlySizedEntries = (entries: number) => {
		...
		// Verify size.
		expect(layoutManager.unpinnedLayoutEntriesSize).toBe(size + entrySizes[0]);
	...
	const verifyFixedSizedPredefinedEntries = (entrySize: number, entries: number) => {
		...
		expect(!layoutManager.getLayoutEntry(entries)).toBeTruthy();
		expect(layoutManager.getLayoutEntry(0)).toEqual({
			index: 0,
			start: 0,
			size: entrySize,
			end: entrySize
		});
		expect(layoutManager.getLayoutEntry(entries - 1)).toEqual({
			index: entries - 1,
		...
		// Verify the size.
		expect(layoutManager.unpinnedLayoutEntriesSize).toBe((entrySize * (entries - 1)) + layoutOverride);
	};
