...
import { ActiveRuntimeSessionMetadata, EnvironmentVariableAction, LanguageRuntimeDynState, LanguageRuntimePackage, PackageInstallRequest, RuntimeSessionMetadata, type notebooks } from 'positron';
...
export interface ExtHostLanguageRuntimeShape {
	...
	$installPackages(handle: number, packages: PackageInstallRequest[]): Promise<void>;
	$uninstallPackages(handle: number, packageNames: string[]): Promise<void>;
	$updatePackages(handle: number, packages: PackageInstallRequest[]): Promise<void>;
