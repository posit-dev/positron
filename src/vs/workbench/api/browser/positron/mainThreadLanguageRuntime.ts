...
class ExtHostLanguageRuntimePackageManagerAdapter implements ILanguageRuntimePackageManager {
	...
	packageRepositoryUrl(token: CancellationToken): Promise<string | undefined> {
		return this._proxy.$packageRepositoryUrl(this._handle, token);
	}

	repositoryRequest(request: IPackageRepositoryRequest, token: CancellationToken): Promise<IPackageRepositoryResponse> {
		return this._proxy.$packageRepositoryRequest(this._handle, request, token);
	}
...
export class ExtHostLanguageRuntimeSessionAdapter extends Disposable implements ILanguageRuntimeSession {
	...
	private readonly _onDidReceiveRuntimeMessageExecutionRequestedEmitter = new Emitter<ILanguageRuntimeMessageExecutionRequested>();
	...
	onDidReceiveRuntimeMessageExecutionRequested = this._onDidReceiveRuntimeMessageExecutionRequestedEmitter.event;
	...
	emitDidReceiveRuntimeMessageExecutionRequested(message: ILanguageRuntimeMessageExecutionRequested) {
		this._onDidReceiveRuntimeMessageExecutionRequestedEmitter.fire(message);
	}
	...
	private processMessage(message: ILanguageRuntimeMessage): void {
		...
		switch (message.type) {
			...
			case LanguageRuntimeMessageType.ExecutionRequested:
				this.emitDidReceiveRuntimeMessageExecutionRequested(
					message as ILanguageRuntimeMessageExecutionRequested);
				break;
