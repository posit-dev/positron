...
export class MainThreadPreviewPanel extends Disposable implements extHostProtocol.MainThreadPreviewPanelShape {
	...
	$previewUrl(extensionData: WebviewExtensionDescription, handle: string, uri: URI): void {

		const extension = reviveWebviewExtension(extensionData);
		const targetUri = URI.revive(uri);
		const origin = this.webviewOriginStore.getOrigin('positron.previewUrl', extension.id);
		const preview = this._positronPreviewService.openUrl(handle, origin, extension, targetUri);
