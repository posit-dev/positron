...
export class RuntimeSessionService extends Disposable implements IRuntimeSessionService, IOpener {
	...
	// A map of the currently active console sessions.
	// We can have multiple console sessions per runtime so this map returns an array.
	...
	async deleteSession(sessionId: string): Promise<boolean> {
		...
		if (this._activeSessionsBySessionId.delete(sessionId)) {
			...
			if (session.metadata.sessionMode === LanguageRuntimeSessionMode.Notebook &&
				session.metadata.notebookUri) {
				this.deleteNotebookSession(session.metadata.notebookUri);
			}
