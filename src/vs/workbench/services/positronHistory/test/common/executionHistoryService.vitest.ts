...
describe('ExecutionHistoryService', () => {
	...
	it('should delete session history when session ends with shutdown reason', () => {
		...
		expect(storageSpy.calledWith(sinon.match(/positron\.executionHistory\.test-session-8/), null)).toBeTruthy();
		expect(storageSpy.calledWith(sinon.match(/positron\.inputHistory\.test-session-8/), null)).toBeTruthy();
	});
	...
	it('should prune storage for inactive sessions', () => {
		...
		expect(removeSpy.calledWith(inactiveKey, StorageScope.WORKSPACE)).toBeTruthy();
		expect(!removeSpy.calledWith(activeKey, StorageScope.WORKSPACE)).toBeTruthy();
	});
	...
	it('should delete session history on restore failure', () => {
		...
		expect(removeSpy.calledWith(sinon.match(new RegExp(`positron\\..*\\.${sessionId}`)), StorageScope.WORKSPACE)).toBeTruthy();
	});
});
