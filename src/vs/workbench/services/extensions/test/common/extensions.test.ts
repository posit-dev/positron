...
// PWB overrides isProposedApiEnabled() in extensions.ts to always return true (Workbench
// curates/trusts extensions, so all proposed APIs are allowed). That override bypasses the
// experiment-gated fallback these upstream tests verify, so the whole suite cannot pass under
